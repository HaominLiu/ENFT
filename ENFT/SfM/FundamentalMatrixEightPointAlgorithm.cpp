...
bool FundamentalMatrixEightPointAlgorithm::Run(const EightMatches2D &data, FundamentalMatrix &F, AlignedVector<__m128> &work)
{
	// AT x vec(FT) = vec(-1)
	// Column-major for LAPACK

	__m128 *AT = (__m128 *) &m_AT8;
	//m_AT8xX.Resize(8);
	//__m128 *AT = (__m128 *) m_AT8xX[0];

	// Column 0: u1u2
	AT[0] = _mm_mul_ps(data.u1_0123(), data.u2_0123());
	AT[1] = _mm_mul_ps(data.u1_4567(), data.u2_4567());
	// Column 1: v1u2
	AT[2] = _mm_mul_ps(data.v1_0123(), data.u2_0123());
	AT[3] = _mm_mul_ps(data.v1_4567(), data.u2_4567());
	// Column 2: u2
	AT[4] = data.u2_0123();
	AT[5] = data.u2_4567();
	// Column 3: u1v2
	AT[6] = _mm_mul_ps(data.u1_0123(), data.v2_0123());
	AT[7] = _mm_mul_ps(data.u1_4567(), data.v2_4567());
	// Column 4: v1v2
	AT[8] = _mm_mul_ps(data.v1_0123(), data.v2_0123());
	AT[9] = _mm_mul_ps(data.v1_4567(), data.v2_4567());
...
bool FundamentalMatrixEightPointAlgorithm::Run(const MatchSet2D &data, FundamentalMatrix &F, AlignedVector<__m128> &work)
{
	m_AT8xX.Resize(data.Size());
	m_b.Resize(data.Size());
	__m128 *AT[8] = {(__m128 *) m_AT8xX[0], (__m128 *) m_AT8xX[1], (__m128 *) m_AT8xX[2], (__m128 *) m_AT8xX[3], 
					 (__m128 *) m_AT8xX[4], (__m128 *) m_AT8xX[5], (__m128 *) m_AT8xX[6], (__m128 *) m_AT8xX[7]};
	__m128 *b = (__m128 *) m_b.Data();
	...
	for(ushort iu1 = 0, iv1 = 1, iu2 = 2, iv2 = 3; iu1 < nPacks; iu1 += 4, iv1 += 4, iu2 += 4, iv2 += 4)
	{
		...
		// Column 0: u1u2
		*AT[0] = _mm_mul_ps(u1, u2);		++AT[0];
		// Column 1: v1u2
		*AT[1] = _mm_mul_ps(v1, u2);		++AT[1];
		// Column 2: u2
		*AT[2] = u2;						++AT[2];
		// Column 3: u1v2
		*AT[3] = _mm_mul_ps(u1, v2);		++AT[3];
		// Column 4: v1v2
		*AT[4] = _mm_mul_ps(v1, v2);		++AT[4];
		// Column 5: v2
		*AT[5] = v2;						++AT[5];
		// Column 6: u1
		*AT[6] = u1;						++AT[6];
...
//	F.M_00_01_02_x() = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(us0), m_vt.M_00_01_02_x()), _mm_mul_ps(_mm_set1_ps(us1), m_vt.M_10_11_12_x()));
//	us0 = m_u.M10() * m_s[0];		us1 = m_u.M11() * m_s[1];
//	F.M_10_11_12_x() = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(us0), m_vt.M_00_01_02_x()), _mm_mul_ps(_mm_set1_ps(us1), m_vt.M_10_11_12_x()));
//	us0 = m_u.M20() * m_s[0];		us1 = m_u.M21() * m_s[1];
//	F.M_20_21_22_x() = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(us0), m_vt.M_00_01_02_x()), _mm_mul_ps(_mm_set1_ps(us1), m_vt.M_10_11_12_x()));
//}