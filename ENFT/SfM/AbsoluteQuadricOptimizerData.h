...
//#endif
	...
	{
		Qnew.v0123() = _mm_add_ps(_mm_mul_ps(s.v0123(), x.v0123()), Qold.v0123());
...
protected:
	...
	static inline void ComputeCoefficientPQPT(const __m128 &pi, const __m128 &pj, LA::AlignedVector6f &cij, __m128 &work)
	{
		work = _mm_mul_ps(pi, pj);
		cij.v0() = work.m128_f32[0] + work.m128_f32[1];
		cij.v5() = work.m128_f32[2];
		cij.v4() = work.m128_f32[3];
		work = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(pi.m128_f32[3]), pj), _mm_mul_ps(pi, _mm_set1_ps(pj.m128_f32[3])));
	...
	static inline void ComputeEquationPQPT(const float &w, const LA::AlignedVector6f &c1, const LA::AlignedVector6f &c2, const float &d1, const float &d2I, 
										   const float &v, float &e, __m128 &j0123, float &j4)
	{
		j4 = w * d2I;
		e = d1 * d2I;
		j0123 = _mm_mul_ps(_mm_set1_ps(j4), _mm_sub_ps(c1.v0123(), _mm_mul_ps(_mm_set1_ps(e), c2.v0123())));
		j4 = j4 * (c1.v4() - e * c2.v4());
		e = w * (v - e);
#ifdef _DEBUG_JACOBIAN_Q
