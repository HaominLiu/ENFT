...
public:
	...
	{
		Pnew.ABCD() = _mm_add_ps(_mm_mul_ps(s.v0123(), x.v0123()), Pold.ABCD());
		Pnew.Normalize();
	}
...
public:
	...
	virtual void ConstructNormalEquation(const Plane &P, LA::AlignedMatrix4f &A, LA::AlignedVector4f &b, LA::AlignedVector4f &s)
	{
		...
		for(uint iPt = 0; iPt < nPts; ++iPt)
		{
			...
			work = _mm_set1_ps(-P.A());		JXpP.M_00_01_02_03() = _mm_mul_ps(work, X.XYZx());	JXpP.M00() -= d;
			work = _mm_set1_ps(-P.B());		JXpP.M_10_11_12_13() = _mm_mul_ps(work, X.XYZx());	JXpP.M11() -= d;
			work = _mm_set1_ps(-P.C());		JXpP.M_20_21_22_23() = _mm_mul_ps(work, X.XYZx());	JXpP.M22() -= d;
			const uint iMea1 = m_mapPtToMea[iPt], iMea2 = m_mapPtToMea[iPt + 1];
			for(uint iMea = iMea1; iMea < iMea2; ++iMea)
			{
				...
				JxXp.M_00_01_02_x() = _mm_mul_ps(ZcI, _mm_sub_ps(C.r_00_01_02_x(), _mm_mul_ps(_mm_set1_ps(x.x()), C.r_20_21_22_x())));
				JxXp.M_10_11_12_x() = _mm_mul_ps(ZcI, _mm_sub_ps(C.r_10_11_12_x(), _mm_mul_ps(_mm_set1_ps(x.y()), C.r_20_21_22_x())));
				LA::AB(JxXp, JXpP, JxP);
