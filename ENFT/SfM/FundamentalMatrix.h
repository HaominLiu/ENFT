...
public:
	...
	bool EnforceSingularConstraint(__m128 *work11);
	bool ToRelativeProjectiveMatrix(ProjectiveMatrix &P, __m128 *work2) const;
	inline void Denormalize(const __m128 &mean_u1v1u2v2, const float &scale1, const float &scale2, __m128 *work6)
	{
		work6[0] = _mm_set1_ps(scale2);
		work6[1] = _mm_set1_ps(scale1);
		work6[2] = _mm_mul_ps(M_00_01_02_x(), work6[0]);
		work6[3] = _mm_mul_ps(M_10_11_12_x(), work6[0]);
		M_00_01_02_x() = _mm_mul_ps(_mm_movelh_ps(work6[2], work6[3]), work6[1]);
		work6[4] = _mm_mul_ps(M_00_01_02_x(), _mm_unpackhi_ps(mean_u1v1u2v2, mean_u1v1u2v2));
		work6[5] = _mm_mul_ps(M_00_01_02_x(), _mm_movelh_ps(mean_u1v1u2v2, mean_u1v1u2v2));
		memcpy(&M10(), &M02(), 8);
		M02() = work6[2].m128_f32[2] - work6[5].m128_f32[0] - work6[5].m128_f32[1];
		M12() = work6[3].m128_f32[2] - work6[5].m128_f32[2] - work6[5].m128_f32[3];
		work6[5] = _mm_mul_ps(_mm_set_ps(work6[3].m128_f32[2], work6[2].m128_f32[2], M21(), M20()), _mm_movelh_ps(work6[1], _mm_movehl_ps(mean_u1v1u2v2, mean_u1v1u2v2)));
	...
	inline void Normalize(const __m128 &mean_u1v1u2v2, const float &scale1, const float &scale2, __m128 *work8)
	{
		work8[0] = _mm_mul_ps(_mm_movelh_ps(_mm_set1_ps(-scale1), _mm_set1_ps(-scale2)), mean_u1v1u2v2);
		work8[1].m128_f32[0] = 1 / scale1;
		work8[1].m128_f32[1] = 1 / scale2;
		__m128 *work6 = work8 + 2;
		Denormalize(work8[0], work8[1].m128_f32[0], work8[1].m128_f32[1], work6);
	}
	inline void GetSSE(__m128 *F) const
	{
	...
	{
		l1.v012x() = _mm_add_ps(_mm_mul_ps(M_00_01_02_x(), _mm_set1_ps(x2.x())), _mm_add_ps(_mm_mul_ps(M_10_11_12_x(), _mm_set1_ps(x2.y())), M_20_21_22_x()));
	}
	...
	static inline void ComputeSymmetricSquaredError4(const __m128 *F, const __m128 &u1, const __m128 &v1, const __m128 &u2, const __m128 &v2, __m128 &errSq, 
													 __m128 *work6)
	{
		/*__m128 l2x*/work6[0] = _mm_add_ps(_mm_add_ps(_mm_mul_ps(F[0], u1), _mm_mul_ps(F[1], v1)), F[2]);
		/*__m128 l2y*/work6[1] = _mm_add_ps(_mm_add_ps(_mm_mul_ps(F[3], u1), _mm_mul_ps(F[4], v1)), F[5]);
		/*__m128 d  */work6[2] = _mm_add_ps(_mm_add_ps(_mm_add_ps(_mm_mul_ps(u2, /*l2x*/F[9]), _mm_mul_ps(v2, /*l2y*/work6[1])), 
													   _mm_add_ps(_mm_mul_ps(F[6], u1), _mm_mul_ps(F[7], v1))), F[8]);
		/*__m128 d2 */work6[3] = _mm_mul_ps(/*d, d*/work6[2], work6[2]);
		/*__m128 l1x*/work6[4] = _mm_add_ps(_mm_add_ps(_mm_mul_ps(F[0], u2), _mm_mul_ps(F[3], v2)), F[6]);
		/*__m128 l1y*/work6[5] = _mm_add_ps(_mm_add_ps(_mm_mul_ps(F[1], u2), _mm_mul_ps(F[4], v2)), F[7]);
		errSq = _mm_add_ps(_mm_div_ps(/*d2*/work6[3], _mm_add_ps(_mm_mul_ps(/*l1x, l1x*/work6[4], work6[4]), _mm_mul_ps(/*l1y, l1y*/work6[5], work6[5]))), 
						   _mm_div_ps(/*d2*/work6[3], _mm_add_ps(_mm_mul_ps(/*l2x, l2x*/work6[0], work6[0]), _mm_mul_ps(/*l2y, l2y*/work6[1], work6[1]))));
	}
