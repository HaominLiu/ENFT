...
public:
	...
	virtual void VerifyModel(const TSEData &data, const TSEModel &model, const std::vector<bool> &inlierMarks, double &fitErr)
	{
		...
		m_work.Resize(4);
		__m128 &st = m_work[0], &RX1pst = m_work[1], &RX2pst = m_work[2], &e = m_work[3], &sqErr = m_work[3], &work1 = m_work[3];
		st = _mm_mul_ps(data.GetTranslation().v012x(), _mm_set1_ps(model));
		...
		{
			RX1pst = _mm_add_ps(data.X(ix2).XYZx(), st);		work1.m128_f32[0] = work1.m128_f32[1] = 1 / RX1pst.m128_f32[2];
			RX2pst = _mm_add_ps(data.X(ix2p1).XYZx(), st);		work1.m128_f32[2] = work1.m128_f32[3] = 1 / RX2pst.m128_f32[2];
			e = _mm_sub_ps(*px2, _mm_mul_ps(_mm_movelh_ps(RX1pst, RX2pst), work1));
			sqErr = _mm_mul_ps(e, e);
			if(inlierMarks[ix2])
		...
		if(_N != N)
		{
			RX1pst = _mm_add_ps(data.X(_N).XYZx(), st);
