...
public:
	...
	inline void ScaleWeights(const float &scale)
	{
		...
		if(AreWeightsValid())
		{
			...
			__m128 *w = (__m128 *) m_ws.Data(), *wSq = (__m128 *) m_wsSq.Data();
	...
	virtual double ComputeSSE(const Point3D &X)
	{
		...
		if(AreWeightsValid())
		{
			for(ushort ix2 = 0, ix2p1 = 1; ix2 < _N; ix2 += 2, ix2p1 += 2, ++px2)
			{
				...
				e2 = _mm_mul_ps(_mm_setr_ps(m_ws[ix2], m_ws[ix2], m_ws[ix2p1], m_ws[ix2p1]), e2);
	...
	virtual void ConstructNormalEquation(const Point3D &X, LA::AlignedMatrix3f &A, LA::AlignedVector3f &b, LA::AlignedVector3f &s)
	{
		...
		if(AreWeightsValid())
		{
			...
			for(ushort i = 0; i < N; ++i)
			{
				...
				J.M_00_01_02_x() = _mm_mul_ps(wZcI, _mm_sub_ps(C.r_00_01_02_x(), _mm_mul_ps(_mm_set1_ps(x.x()), C.r_20_21_22_x())));
				J.M_10_11_12_x() = _mm_mul_ps(wZcI, _mm_sub_ps(C.r_10_11_12_x(), _mm_mul_ps(_mm_set1_ps(x.y()), C.r_20_21_22_x())));
				LA::AmB(m_xs[i], x, e);
				e *= m_ws[i];
		...
		else
		{
			...
			for(ushort i = 0; i < N; ++i)
			{
				...
				J.M_00_01_02_x() = _mm_mul_ps(ZcI, _mm_sub_ps(C.r_00_01_02_x(), _mm_mul_ps(_mm_set1_ps(x.x()), C.r_20_21_22_x())));
				J.M_10_11_12_x() = _mm_mul_ps(ZcI, _mm_sub_ps(C.r_10_11_12_x(), _mm_mul_ps(_mm_set1_ps(x.y()), C.r_20_21_22_x())));
				LA::AmB(m_xs[i], x, e);
				LA::AddATAToUpper(J, A);
				LA::AddATBTo(J, e, b);
				LA::AddAij2To(J, s);
	...
	{
		Xnew.XYZx() = _mm_add_ps(_mm_mul_ps(s.v012x(), x.v012x()), Xold.XYZx());
		Xnew.reserve() = 1.0f;
	}
