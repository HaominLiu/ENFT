...
public:
	...
	inline void Run(TranslationScaleEstimatorData &data, float &scale, const ushort &maxNumIters, AlignedVector<__m128> &work, const bool &verbose = false)
	{
...
private:
	...
	inline void Compute_hx(const float* const &p)
	{
		...
		__m128 &st = m_work2m[0], &RXpst = m_work2m[1];
		st = _mm_mul_ps(data.GetTranslation().v012x(), _mm_set1_ps(m_scale));
		const ushort N = data.Size();
		float *intermediateResult = m_intermediateResults, *x = m_xs;
		for(ushort i = 0; i < N; ++i, ++intermediateResult, x += 2)
		{
			RXpst = _mm_add_ps(data.X(i).v012x(), st);
