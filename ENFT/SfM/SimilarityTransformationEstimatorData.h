...
public:
	...
	virtual void ConstructNormalEquation(const SimilarityTransformation3D &S, LA::AlignedMatrix7f &A, LA::AlignedVector7f &b, LA::AlignedVector7f &s)
	{
		...
		__m128 &RX = m_work[3], &TX = m_work[4], &SX = m_work[5], &sRX = m_work[6], &e = m_work[7];
		A.SetZero();
		b.SetZero();
		s.SetZero();
		const uint N = m_X1s.Size();
		...
		A.SetLowerFromUpper();
		LA::FinishAdditionATBTo<LA::AlignedMatrix3x7f, __m128>(b);
...
public:
	...
	{
		uint i, j;
		__m128 &RX = m_work[3], &TX = m_work[4], &SX = m_work[5], &sRX = m_work[6], &ZcI = m_work[7], &wZcI = m_work[7], &JxSX = m_work[8], &JySX = m_work[9];
		Point2D x, &e = x;
		A.SetZero();
		b.SetZero();
		s.SetZero();
		...
		if(AreWeightsValid())
		{
			for(i = 0; i < nPts; ++i)
			{
				...
				for(j = j1; j < j2; ++j)
				{
					...
					JxSX = _mm_sub_ps(_mm_mul_ps(wZcI, C.r_00_01_02_x()), _mm_mul_ps(_mm_set1_ps(wZcI.m128_f32[0] * x.x()), C.r_20_21_22_x()));
					JySX = _mm_sub_ps(_mm_mul_ps(wZcI, C.r_10_11_12_x()), _mm_mul_ps(_mm_set1_ps(wZcI.m128_f32[0] * x.y()), C.r_20_21_22_x()));
					m_J.M00() = SSE::Sum012(_mm_mul_ps(JxSX, TX));
					...
					m_J.M_04_05_06_x() = _mm_mul_ps(S.sss1(), JxSX);
					m_J.M10() = SSE::Sum012(_mm_mul_ps(JySX, TX));
					...
					m_J.M_14_15_16_x() = _mm_mul_ps(S.sss1(), JySX);
					LA::AmB(m_x2s[j], x, e);
					e *= m_ws[i];
					LA::AddATAToUpper(m_J, A, m_work);
					LA::AddATBTo(m_J, e, b, m_work);
		...
		else
		{
			for(i = 0; i < nPts; ++i)
			{
				...
				for(j = j1; j < j2; ++j)
				{
					...
					JxSX = _mm_sub_ps(_mm_mul_ps(ZcI, C.r_00_01_02_x()), _mm_mul_ps(_mm_set1_ps(ZcI.m128_f32[0] * x.x()), C.r_20_21_22_x()));
					JySX = _mm_sub_ps(_mm_mul_ps(ZcI, C.r_10_11_12_x()), _mm_mul_ps(_mm_set1_ps(ZcI.m128_f32[0] * x.y()), C.r_20_21_22_x()));
					m_J.M00() = SSE::Sum012(_mm_mul_ps(JxSX, TX));
					m_J.M01() = JxSX.m128_f32[2] * sRX.m128_f32[1] - JxSX.m128_f32[1] * sRX.m128_f32[2];
					m_J.M02() = JxSX.m128_f32[0] * sRX.m128_f32[2] - JxSX.m128_f32[2] * sRX.m128_f32[0];
					m_J.M03() = JxSX.m128_f32[1] * sRX.m128_f32[0] - JxSX.m128_f32[0] * sRX.m128_f32[1];
