...
public:
	...
	inline void SetArsacData(const ushort &width, const ushort &height)
	{
		...
		const __m128 cx_cy_cx_cy = _mm_setr_ps(cx.x(), cx.y(), cx.x(), cx.y());
		const uint N = uint(m_xs.Size()), _N = N - (N & 1);
		__m128 *p2 = (__m128 *) m_xs.Data();
		for(uint i = 0; i < _N; i += 2, ++p2)
			*p2 = _mm_sub_ps(*p2, cx_cy_cx_cy);
		if(_N != N)
			m_xs[_N] -= cx;
	}
	...
	{
		Tnew.tXYZ() = _mm_add_ps(_mm_mul_ps(s.v012x(), x.v012x()), Told.tXYZ());
	}
