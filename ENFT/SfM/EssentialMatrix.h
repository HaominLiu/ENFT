...
public:
	...
	//}
	inline void ToFundamentalMatrix(const IntrinsicMatrix &K1, const IntrinsicMatrix &K2, FundamentalMatrix &F, __m128 *work3) const
	{
		IntrinsicMatrix::K1Tinv_M_K2inv(K2, *this, K1, F, work3);
	}
	bool ToRelativePose(const MatchSet2D &xs, RigidTransformation3D &T, const float &sccRatioTh, __m128 *work38) const;

	static void SolveIdealPoint(const EssentialMatrix &E, const RigidTransformation3D &T, const LA::Vector2f &x1, const LA::Vector2f &x2, __m128 &X);
	static void SolveIdealPoint(const EssentialMatrix &E, const RigidTransformation3D &T, const __m128 &x1x2, __m128 &X);
	static void CheckCheirality(const EssentialMatrix &E, const RigidTransformation3D &T, const MatchSet2D &xs, ushort &cntPos, ushort &cntNeg, __m128 *work28);
	static bool CheckCheirality(const EssentialMatrix &E, const RigidTransformation3D &T, const MatchSet2D &xs, __m128 *work28);
