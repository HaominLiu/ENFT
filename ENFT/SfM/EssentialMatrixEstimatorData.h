...
public:
	...
	virtual void ConstructNormalEquation(const RigidTransformation3D &T, LA::AlignedCompactMatrix6f &A, LA::AlignedVector6f &b, LA::AlignedVector6f &s)
	{
		...
		__m128 &one = m_work[16], &zero = m_work[17], *je = &m_work[18], &e = m_work[26];
		zero = _mm_setzero_ps();
		...
		for(ushort iu1 = 0, iv1 = 1, iu2 = 2, iv2 = 3; iu1 < nPacks; iu1 += 4, iv1 += 4, iu2 += 4, iv2 += 4)
		{
			...
			je[0] = _mm_mul_ps(jlx, u1);		je[1] = _mm_mul_ps(jlx, v1);		je[2] = jlx;
			je[3] = _mm_mul_ps(jly, u1);		je[4] = _mm_mul_ps(jly, v1);		je[5] = jly;
			je[6] = _mm_mul_ps(jlz, u1);		je[7] = _mm_mul_ps(jlz, v1);
