	...
	inline void Initialize(const std::vector<LA::Vector2us> &octaveSizes, const float &dogTh, const float &edgeTh)
	{
		...
		for(ushort iOctave = 0; iOctave < nOctaves; ++iOctave)
		{
			...
			out <<
				...
				"		extremum.rb = vec2(0);\n"
				"	if(gl_FragCoord.s > " << octaveSizes[iOctave].v0() - 1 << ")\n"
				"		extremum.ga = vec2(0);\n"
				"	if(gl_FragCoord.t < 1.0)\n"
				"		extremum.rg = vec2(0);\n"
				"	if(gl_FragCoord.t > " << octaveSizes[iOctave].v1() - 1 << ")\n"
				"		extremum.ba = vec2(0);\n"
				"	gl_FragColor = vec4(0);\n"
			...
			out <<
				...
				"		if( score_down <= 0 || score_up > " << (edgeTh+1)*(edgeTh+1)/edgeTh << " * score_down)extremum[i] = 0;\\\n"
				"	}\n"
