...
namespace LA
{
	...
	public:
		...
		inline const __m128& v45xx()   const { return m_45xx; }						inline __m128& v45xx()	 { return m_45xx; }
		...
		inline void MakeReciprocal()
		{
			...
			m_45xx = _mm_and_ps(_mm_cmpneq_ps(m_45xx, _mm_setzero_ps()), _mm_div_ps(_mm_set1_ps(1), m_45xx));
		}
		inline void MakeSquareRoot()
		{
			m_0123 = _mm_sqrt_ps(m_0123);
			m_45xx = _mm_sqrt_ps(m_45xx);
		}
		inline float SquaredLength() const
		{
			return SSE::Sum0123(_mm_mul_ps(m_0123, m_0123)) + v4() * v4() + v5() * v5();
		}
		inline float Dot(const AlignedVector6f &v) const
		{
			return SSE::Sum0123(_mm_mul_ps(v0123(), v.v0123())) + v4() * v.v4() + v5() * v.v5();
		}
		inline float Dot(const AlignedVector5f &v) const
		{
			return SSE::Sum0123(_mm_mul_ps(v0123(), v.v0123())) + v4() * v.v4() + v5();
		}
	...
	{
		return SSE::Sum0123(_mm_mul_ps(v.v0123(), _mm_mul_ps(v.v0123(), w.v0123()))) + v.v4() * v.v4() * w.v4() + v.v5() * v.v5() * w.v5();
	}
	...
	{
		sA.v0123() = _mm_mul_ps(_mm_set1_ps(s), A.v0123());
		sA.v4() = s * A.v4();
		sA.v5() = s * A.v5();
	}
	inline void sA(const __m128 &s, AlignedVector6f &A)
	{
		A.v0123() = _mm_mul_ps(s, A.v0123());
		A.v4() *= s.m128_f32[0];
		A.v5() *= s.m128_f32[0];
	}
	inline void sA(const __m128 &s, const AlignedVector6f &A, AlignedVector6f &sA)
	{
	...
	inline void sA(const AlignedVector6f &s, AlignedVector6f &A)
	{
		...
		A.v45xx() = _mm_mul_ps(s.v45xx(), A.v45xx());
	}
	...
	inline void sApB(const __m128 &s, const AlignedVector6f &A, const AlignedVector6f &B, AlignedVector6f &sApB)
	{
	...
	{
		to.v0123() = _mm_add_ps(_mm_mul_ps(A.v0123(), _mm_set1_ps(B)), to.v0123());
		to.v4() = A.v4() * B + to.v4();
		to.v5() = A.v5() * B + to.v5();
	}
	inline void AddAij2To(const AlignedVector6f &A, AlignedVector6f &to)
	{
		to.v0123() = _mm_add_ps(_mm_mul_ps(A.v0123(), A.v0123()), to.v0123());
