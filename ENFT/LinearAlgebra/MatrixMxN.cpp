...
//	AAT.M##i##j() = SSE::Sum0123(_mm_mul_ps(PTR_TO_SSE(pAi  ), PTR_TO_SSE(pAj))) + \
//					SSE::Sum0123(_mm_mul_ps(PTR_TO_SSE(pAi+4), PTR_TO_SSE(pAj+4)))
//#endif
//
...
{
	const __m128 *pAi, *pAj;
	float dot;

#ifndef SET_AAT_ENTRY_3xN
#define SET_AAT_ENTRY_3xN(i, j)\
	pAi = (const __m128 *) A[i];	pAj = (const __m128 *) A[j];	dot = 0;\
	for(uint k = 0; k < _N; k += 4, ++pAi, ++pAj)\
		dot += SSE::Sum0123(_mm_mul_ps(*pAi, *pAj));\
	for(uint k = _N; k < N; k++)\
		dot += A[i][k] * A[j][k];\
	AAT.M##i##j() = dot
#endif
