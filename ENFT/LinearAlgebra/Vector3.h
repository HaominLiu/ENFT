...
namespace LA
{
	...
	public:
		...
		inline AlignedVector3f(const float &v0, const float &v1, const float &v2){ m_v012x = _mm_setr_ps(v0, v1, v2, 1.0f); }
		inline AlignedVector3f(const __m128 &v012x) : m_v012x(v012x) {}

		inline const __m128 &v012x () const { return m_v012x; }				inline __m128 &v012x () { return m_v012x; }
		...
		//	return VALUE_FROM_SSE(AlignedVector3f, _mm_add_ps(m_v012x, v.m_v012x));
		//}
		inline void operator += (const __m128 &v012x) { m_v012x = _mm_add_ps(m_v012x, v012x); }
		inline void operator += (const AlignedVector3f &v) { m_v012x = _mm_add_ps(m_v012x, v.m_v012x); }
		inline AlignedVector3f operator - (const AlignedVector3f &v) { return AlignedVector3f(_mm_sub_ps(m_v012x, v.m_v012x)); }
		inline void operator -= (const __m128 &v012x) { m_v012x = _mm_sub_ps(m_v012x, v012x); }
		inline void operator -= (const AlignedVector3f &v) { m_v012x = _mm_sub_ps(m_v012x, v.m_v012x); }
		inline void operator *= (const float &s) { m_v012x = _mm_mul_ps(m_v012x, _mm_set1_ps(s)); }
		inline void operator *= (const __m128 &s) { m_v012x = _mm_mul_ps(m_v012x, s); }

		inline void SetZero() { memset(this, 0, 16); }
		inline void Set(const float &v0, const float &v1, const float &v2) { m_v012x = _mm_setr_ps(v0, v1, v2, 1.0f); }
		inline void Set(const double *v012) { Set(float(v012[0]), float(v012[1]), float(v012[2])); }
		inline void Set(const __m128 &v012x) { m_v012x = v012x; }
		inline void Get(double *v012) const { v012[0] = double(v0()); v012[1] = double(v1()); v012[2] = double(v2()); }
		inline float Length() const { return sqrt(SSE::Sum012(_mm_mul_ps(m_v012x, m_v012x))); }
		inline float SquaredLength() const { return SSE::Sum012(_mm_mul_ps(m_v012x, m_v012x)); }
		inline float SquaredDistance(const AlignedVector3f &v) const
		{
			return SSE::Sum012(_mm_mul_ps(_mm_sub_ps(m_v012x, v.m_v012x), _mm_sub_ps(m_v012x, v.m_v012x)));
		}
		inline void Normalize() { m_v012x = _mm_mul_ps(_mm_set1_ps(1 / sqrt(SSE::Sum012(_mm_mul_ps(m_v012x, m_v012x)))), m_v012x); }
		inline const float MaximalElement() const { return std::max(std::max(v0(), v1()), v2()); }
		inline float Dot(const AlignedVector3f &v) const { return SSE::Sum012(_mm_mul_ps(m_v012x, v.m_v012x)); }
		inline void Scale(const float &s) { m_v012x = _mm_mul_ps(_mm_set1_ps(s), m_v012x); }
		inline void Scale(const __m128 &s) { m_v012x = _mm_mul_ps(s, m_v012x); }

		inline void MakeReciprocal() { m_v012x = _mm_and_ps(_mm_cmpneq_ps(m_v012x, _mm_setzero_ps()), _mm_div_ps(_mm_set1_ps(1.0f), m_v012x)); }
		inline void MakeSquareRoot() { m_v012x = _mm_sqrt_ps(m_v012x); }
	...
	inline void Cross(const __m128 &A, const __m128 &B, __m128 &AcB)
	{
	...
	{
		const __m128 e = _mm_sub_ps(A.v012x(), B.v012x());
		absErr = sqrt(SSE::Sum012(_mm_mul_ps(e, e)));
	...
	inline void AddAij2To(const AlignedVector3f &A, Vector3f &to, __m128 &work)
	{
		work = _mm_mul_ps(A.v012x(), A.v012x());
		to.v0() = work.m128_f32[0] + to.v0();
		to.v1() = work.m128_f32[1] + to.v1();
		to.v2() = work.m128_f32[2] + to.v2();
	}
	inline void AddsATo(const float &s, const AlignedVector3f &A, Vector3f &to, __m128 &work)
	{
		work = _mm_mul_ps(_mm_set1_ps(s), A.v012x());
		to.v0() = work.m128_f32[0] + to.v0();
		to.v1() = work.m128_f32[1] + to.v1();
		to.v2() = work.m128_f32[2] + to.v2();
	}
