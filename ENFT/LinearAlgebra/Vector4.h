...
namespace LA
{
	...
	public:
		...
		inline AlignedVector4f(const float &v0, const float &v1, const float &v2, const float &v3) { v0123() = _mm_setr_ps(v0, v1, v2, v3); }

		inline const __m128 &v0123 () const { return m_v0123; }				inline __m128 &v0123 () { return m_v0123; }
		...
		inline void Set(const float &v0, const float &v1, const float &v2, const float &v3) { v0123() = _mm_setr_ps(v0, v1, v2, v3); }
		inline void SetZero() { v0123() = _mm_setzero_ps(); }
		inline void Scale(const float &s) { v0123() = _mm_mul_ps(_mm_set1_ps(s), v0123()); }
		inline void MakeReciprocal() { v0123() = _mm_and_ps(_mm_cmpneq_ps(v0123(), _mm_setzero_ps()), _mm_div_ps(_mm_set1_ps(1), v0123())); }
		inline void MakeSquareRoot() { v0123() = _mm_sqrt_ps(v0123()); }
		inline float SquaredLength() const { return SSE::Sum0123(_mm_mul_ps(v0123(), v0123())); }
		inline float SquaredDistance(const AlignedVector4f &v) const
		{
			return SSE::Sum0123(_mm_mul_ps(_mm_sub_ps(m_v0123, v.m_v0123), _mm_sub_ps(m_v0123, v.m_v0123)));
		}
		inline float Dot(const AlignedVector4f &v) const { return SSE::Sum0123(_mm_mul_ps(v0123(), v.v0123())); }
		inline void Normalize() { m_v0123 = _mm_mul_ps(_mm_set1_ps(1 / sqrt(SSE::Sum0123(_mm_mul_ps(m_v0123, m_v0123)))), m_v0123); }
		inline void operator += (const LA::AlignedVector4f &v) { m_v0123 = _mm_add_ps(m_v0123, v.m_v0123); }
		inline void operator *= (const float &s) { m_v0123 = _mm_mul_ps(m_v0123, _mm_set1_ps(s)); }
	...
	{
		to.v0123() = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(A), B.v0123()), to.v0123());
	}
	...
	{
		return SSE::Sum0123(_mm_mul_ps(v.v0123(), _mm_mul_ps(v.v0123(), w.v0123())));
	}
	inline float Dot(const AlignedVector4f &A, const AlignedVector4f &B) { return A.Dot(B); }
	inline void sA(const float &s, const AlignedVector4f &A, AlignedVector4f &sA) { sA.v0123() = _mm_mul_ps(_mm_set1_ps(s), A.v0123()); }
	inline void sA(const AlignedVector4f &s, AlignedVector4f &A) { A.v0123() = _mm_mul_ps(s.v0123(), A.v0123()); }
	inline void sApB(const float &s, const AlignedVector4f &A, const AlignedVector4f &B, AlignedVector4f &sApB)
	{
		sApB.v0123() = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(s), A.v0123()), B.v0123());
	}
...
#endif
		to.v0123() = _mm_add_ps(_mm_mul_ps(A.v012x(), A.v012x()), to.v0123());
	}
	inline void AddsATo(const float &s, const AlignedVector3f &A, AlignedVector4f &to)
	{
		...
		assert(A.reserve() == 1.0f);
#endif
		to.v0123() = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(s), A.v012x()), to.v0123());
	}
