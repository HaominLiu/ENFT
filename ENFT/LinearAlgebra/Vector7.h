...
namespace LA
{
	...
	public:
		...
		inline const __m128& v456x()  const { return m_456x; }						inline __m128& v456x()	{ return m_456x; }
		...
		{
			m_0123 = _mm_and_ps(_mm_cmpneq_ps(m_0123, _mm_setzero_ps()), _mm_div_ps(_mm_set1_ps(1), m_0123));
			m_456x = _mm_and_ps(_mm_cmpneq_ps(m_456x, _mm_setzero_ps()), _mm_div_ps(_mm_set1_ps(1), m_456x));
		}
		inline void MakeSquareRoot()
		{
			m_0123 = _mm_sqrt_ps(m_0123);
			m_456x = _mm_sqrt_ps(m_456x);
		}
		inline float SquaredLength() const
		{
			...
			assert(reserve() == 0);
#endif
			return SSE::Sum0123(_mm_add_ps(_mm_mul_ps(m_0123, m_0123), _mm_mul_ps(m_456x, m_456x)));
		}
...
#endif
			return SSE::Sum0123(_mm_add_ps(_mm_mul_ps(m_0123, v.m_0123), _mm_mul_ps(m_456x, v.m_456x)));
		}
		inline void Scale(const __m128 &s) { m_0123 = _mm_mul_ps(m_0123, s); m_456x = _mm_mul_ps(m_456x, s); }
...
#endif
		return SSE::Sum0123(_mm_add_ps(_mm_mul_ps(v.v0123(), _mm_mul_ps(v.v0123(), w.v0123())), 
									   _mm_mul_ps(v.v456x(), _mm_mul_ps(v.v456x(), w.v456x()))));
	}
	...
	inline void sA(const AlignedVector7f &s, AlignedVector7f &A)
	{
		...
		A.v456x() = _mm_mul_ps(s.v456x(), A.v456x());
	}
	inline void sA(const __m128 &s, const AlignedVector7f &A, AlignedVector7f &sA)
	{
		sA.v0123() = _mm_mul_ps(s, A.v0123());
		sA.v456x() = _mm_mul_ps(s, A.v456x());
	}
	inline void sApB(const __m128 &s, const AlignedVector7f &A, const AlignedVector7f &B, AlignedVector7f &sApB)
	{
		sApB.v0123() = _mm_add_ps(_mm_mul_ps(s, A.v0123()), B.v0123());
		sApB.v456x() = _mm_add_ps(_mm_mul_ps(s, A.v456x()), B.v456x());
	}
	inline void AmB(const AlignedVector7f &A, const AlignedVector7f &B, AlignedVector7f &AmB)
	{
		AmB.v0123() = _mm_sub_ps(A.v0123(), B.v0123());
		AmB.v456x() = _mm_sub_ps(A.v456x(), B.v456x());
	}
	inline void AddABTo(const float &A, AlignedVector7f &B, AlignedVector7f &to, __m128 &work)
	{
		work = _mm_set1_ps(A);
		to.v0123() = _mm_add_ps(_mm_mul_ps(work, B.v0123()), to.v0123());
		to.v456x() = _mm_add_ps(_mm_mul_ps(work, B.v456x()), to.v456x());
	}
