...
namespace LA
{
	...
	inline void AddAij2To(const Vector3f &A, float &to, __m128 *work0) { AddAij2To(A, to); }
	...
	inline void sA(const __m128 &s, const float &A, float &sA) { sA = s.m128_f32[0] * A; }
	inline void sApB(const float &s, const float &A, const float &B, float &sApB) { sApB = s * A + B; }
	inline void sApB(const __m128 &s, const float &A, const float &B, float &sApB) { sApB = s.m128_f32[0] * A + B; }
	...
	inline void AddATAToUpper(const Vector3f &A, float &to, __m128 *work0) { AddATAToUpper(A, to); }
	...
	inline void ATB(const float &A, const Vector3f &B, Vector3f &ATB, __m128 *work0) { LA::ATB(A, B, ATB); }
	inline void AB(const Vector3f &A, const float &B, AlignedVector3f &AB) { AB.v0() = A.v0() * B; AB.v1() = A.v1() * B; AB.v2() = A.v2() * B; }
	inline void AB(const Vector3f &A, const float &B, AlignedVector3f &AB, __m128 *work0) { LA::AB(A, B, AB); }
	...
	inline void ABpCD(const Vector3f &A, const float &B, const float &C, const Vector3f &D, AlignedVector3f &ABpCD, __m128 *work0) { LA::ABpCD(A, B, C, D, ABpCD); }
	inline void SubtractATBFrom(const Vector3f &A, const Vector3f &B, float &from) { from -= A.Dot(B); }
	inline void SubtractATBFrom(const Vector3f &A, const Vector3f &B, float &from, __m128 *work0) { SubtractATBFrom(A, B, from); }
	...
	inline void SubtractABFrom(const Vector3f &A, const float &B, Vector3f &from, __m128 *work0) { SubtractABFrom(A, B, from); }
	...
	inline void AddATBTo(const Vector3f &A, const AlignedVector3f &B, float &to, __m128 *work0) { AddATBTo(A, B, to); }
	...
	inline void AB(const float &A, const float &B, float &AB, __m128 *work0) { LA::AB(A, B, AB); }
	inline void AddATBTo(const Vector3f &A, const Vector3f &B, float &to) { to = A.Dot(B) + to; }
	inline void AddATBTo(const Vector3f &A, const Vector3f &B, float &to, __m128 *work0) { AddATBTo(A, B, to); }
	inline void AddATBTo(const float &A, const float &B, float &to) { to = A * B + to; }
	inline void AddATBTo(const float &A, const float &B, float &to, __m128 *work0) { AddATBTo(A, B, to); }
	...
	inline void AddABTo(const Vector3f &A, const float &B, Vector3f &to, __m128 *work0) { AddABTo(A, B, to); }
	inline void AddABTo(const float &A, const float &B, float &to) { to = A * B + to; }
	inline void AddABTo(const float &A, const float &B, float &to, __m128 *work0) { AddABTo(A, B, to); }
	inline void ABmC(const float &A, const float &B, const float &C, float &ABmC) { ABmC = A * B - C; }
	inline void ABmC(const float &A, const float &B, const float &C, float &ABmC, __m128 *work0) { LA::ABmC(A, B, C, ABmC); }
}
