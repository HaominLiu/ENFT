...
ViewerSequence::ViewerSequence() : m_pSeq(NULL), m_camVolume(*(CameraVolume *) _aligned_malloc(sizeof(CameraVolume), SSE_ALIGNMENT)), 
								   m_Cg(*(Camera *) _aligned_malloc(sizeof(Camera), SSE_ALIGNMENT)), 
								   m_translationStart(*(Point3D *) _aligned_malloc(sizeof(Point3D), SSE_ALIGNMENT)), 
								   m_translation(*(Point3D *) _aligned_malloc(sizeof(Point3D), SSE_ALIGNMENT)), 
								   m_whereMouseDown3DStart(*(Point3D *) _aligned_malloc(sizeof(Point3D), SSE_ALIGNMENT)), 
...
bool ViewerSequence::PrepareActiveFrame(const FrameIndex iFrmActive)
{
	...
	{
		m_pSeq->ComputeFrameInlierRatioAndMSE(m_iFrmActive, m_nInliers, m_inlierRatio, SSE, m_MSE);

		m_pSeq->GetFrameTrackedFeatureIndexList(m_iFrmActive, m_iFtrs);
		m_pSeq->ComputeFrameFeaturesDistribution(m_iFrmActive, m_iFtrs, m_meanFtrsTrked, m_covFtrsTrked);
		m_pSeq->GetFrameInlierFeatureIndexList(m_iFrmActive, m_iFtrs);
		m_pSeq->ComputeFrameFeaturesDistribution(m_iFrmActive, m_iFtrs, m_meanFtrsInlier, m_covFtrsInlier);
		m_pSeq->GetFrameOutlierFeatureIndexList(m_iFrmActive, m_iFtrs);
		m_pSeq->ComputeFrameFeaturesDistribution(m_iFrmActive, m_iFtrs, m_meanFtrsOutlier, m_covFtrsOutlier);

		m_inlierRatioArea = sqrt((m_covFtrsInlier.v0() * m_covFtrsInlier.v2() - m_covFtrsInlier.v1() * m_covFtrsInlier.v1())
			/ (m_covFtrsTrked.v0() * m_covFtrsTrked.v2() - m_covFtrsTrked.v1() * m_covFtrsTrked.v1()));
	}
	if(m_imgView)
...
void ViewerSequence::DrawImageView()
{
	...
	if(IsFrameSolved(m_iFrmActive))
	{
		...
		{
		case DRAW_NO_PROJECTION:															break;
		case DRAW_ALL_PROJECTIONS:		DrawAllProjections();	DrawActiveProjections();	break;
		case DRAW_ACTIVE_PROJECTIONS:	DrawActiveProjections();							break;
		}
	}
...
void ViewerSequence::DrawFeaturePoints()
{
	...
	const MeasurementIndex iMea1 = m_pSeq->GetFrameFirstMeasurementIndex(m_iFrmActive), iMea2 = m_pSeq->GetFrameFirstMeasurementIndex(m_iFrmActive + 1);
...
void ViewerSequence::DrawFeatureTracks()
{
	...
	if(m_drawTrkType != DRAW_TRACK_TO_NEXT_KEY_FRAME && m_iFrmActive == 0 || m_drawTrkType == DRAW_TRACK_TO_NEXT_KEY_FRAME && m_iFrmActive == nFrms - 1)
		return;
	glBegin(GL_LINES);
	glColor3ub(COLOR_TRACK_TEMPORAL_R, COLOR_TRACK_TEMPORAL_G, COLOR_TRACK_TEMPORAL_B);

	FrameIndex iFrmOther;
	switch(m_drawTrkType)
	{
	case DRAW_TRACK_FROM_LAST_FRAME:		iFrmOther = m_iFrmActive - 1;	break;
	case DRAW_TRACK_FROM_LAST_KEY_FRAME:
		for(iFrmOther = m_iFrmActive - 1; iFrmOther > 0 && !(m_pSeq->GetFrameState(iFrmOther) & FLAG_FRAME_STATE_KEY_FRAME); --iFrmOther);
		break;
	case DRAW_TRACK_TO_NEXT_KEY_FRAME:
		for(iFrmOther = m_iFrmActive + 1; iFrmOther < nFrms && !(m_pSeq->GetFrameState(iFrmOther) & FLAG_FRAME_STATE_KEY_FRAME); ++iFrmOther);
		break;
	}
	...
	for(MeasurementIndex iMea = iMea1; iMea < iMea2; ++iMea)
	{
		if(ShouldMeasurementBeHidden(iMea) || (iTrk = m_pSeq->GetMeasurementTrackIndex(iMea)) == INVALID_TRACK_INDEX
		|| (iMeaOther = m_pSeq->SearchTrackForFrameMeasurementIndex(iTrk, iFrmOther)) == INVALID_MEASUREMENT_INDEX)
			continue;
		else if(m_pSeq->AreMeasurementsNormalized())
		{
			K.NormalizedPlaneToImage(m_pSeq->GetMeasurement(iMeaOther), x);	glVertex2fv(x);
			K.NormalizedPlaneToImage(m_pSeq->GetMeasurement(iMea), x);		glVertex2fv(x);
		}
		else
		{
...
void ViewerSequence::DrawFeatureDistribution()
{
	...
	glColor3ub(COLOR_FEATURE_ELLIPSE_TRACKED_R, COLOR_FEATURE_ELLIPSE_TRACKED_G, COLOR_FEATURE_ELLIPSE_TRACKED_B);
	Viewer::ConvertMeanCovarianceToEllipse(m_meanFtrsTrked, m_covFtrsTrked, m_ellipse);
	Viewer::DrawPoints(m_ellipse);
	glEnd();

	glBegin(GL_LINE_LOOP);
	glColor3ub(COLOR_FEATURE_ELLIPSE_INLIER_R, COLOR_FEATURE_ELLIPSE_INLIER_G, COLOR_FEATURE_ELLIPSE_INLIER_B);
	Viewer::ConvertMeanCovarianceToEllipse(m_meanFtrsInlier, m_covFtrsInlier, m_ellipse);
	Viewer::DrawPoints(m_ellipse);
	glEnd();

	glBegin(GL_LINE_LOOP);
	glColor3ub(COLOR_FEATURE_ELLIPSE_OUTLIER_R, COLOR_FEATURE_ELLIPSE_OUTLIER_G, COLOR_FEATURE_ELLIPSE_OUTLIER_B);
	Viewer::ConvertMeanCovarianceToEllipse(m_meanFtrsOutlier, m_covFtrsOutlier, m_ellipse);
	Viewer::DrawPoints(m_ellipse);
	glEnd();
...
{
	glColor3ub(COLOR_FEATURE_SELECTION_BOX_R, COLOR_FEATURE_SELECTION_BOX_G, COLOR_FEATURE_SELECTION_BOX_B);
	const IntrinsicMatrix &K = m_pSeq->GetIntrinsicMatrix();
	Point2D x1 = m_pSeq->GetFrameFeature(m_iFrmActive, m_iFtrSelected);
	if(m_pSeq->AreMeasurementsNormalized())
		K.NormalizedPlaneToImage(x1);
	if(IsFrameSolved(m_iFrmActive) && m_iTrkSelected != INVALID_TRACK_INDEX && (m_pSeq->GetTrackState(m_iTrkSelected) & FLAG_TRACK_STATE_SOLVED))
	{
		Point2D x2;
		m_pSeq->GetCamera(m_iFrmActive).ProjectToNormalizedPlane(m_pSeq->GetPoint(m_iTrkSelected), x2);
	...
	glLineWidth(2);
	glBegin(GL_LINES);
	const Point2D boxSize(FEATURE_SELECTION_BOX_SIZE * m_factorWinToImg.x(), FEATURE_SELECTION_BOX_SIZE * m_factorWinToImg.y());
...
void ViewerSequence::DrawActiveProjections()
{
	...
	for(MeasurementIndex iMea = iMea1; iMea < iMea2; ++iMea)
	{
		if((iTrk = m_pSeq->GetMeasurementTrackIndex(iMea)) == INVALID_TRACK_INDEX || !(m_pSeq->GetTrackState(iTrk) & FLAG_TRACK_STATE_SOLVED)
		|| ShouldMeasurementBeHidden(iMea))
			continue;
		if(m_drawErrs)
		{
			glBegin(GL_LINES);
			glColor3ub(COLOR_FEATURE_ERROR_LINE_R, COLOR_FEATURE_ERROR_LINE_G, COLOR_FEATURE_ERROR_LINE_B);
			if(m_pSeq->AreMeasurementsNormalized())
				K.NormalizedPlaneToImage(m_pSeq->GetMeasurement(iMea), x);
			else
				x = m_pSeq->GetMeasurement(iMea);
			...
			glBegin(GL_POINTS);
			glColor3ub(COLOR_PROJECTION_ACTIVE_R, COLOR_PROJECTION_ACTIVE_G, COLOR_PROJECTION_ACTIVE_B);
			glVertex2fv(x);
			glEnd();
		}
		else
...
void ViewerSequence::DrawAllProjections()
{
	...
	{
		if(!(m_pSeq->GetTrackState(iTrk) & FLAG_TRACK_STATE_SOLVED) || ShouldTrackBeHidden(iTrk))
			continue;
		...
		K.NormalizedPlaneToImage(x);
		if(Xc.Z() > 0)
			glColor3ub(COLOR_PROJECTION_FRONT_R, COLOR_PROJECTION_FRONT_G, COLOR_PROJECTION_FRONT_B);
		else
			glColor3ub(COLOR_PROJECTION_BACK_R, COLOR_PROJECTION_BACK_G, COLOR_PROJECTION_BACK_B);
		glVertex2fv(x);
	}
...
{
	const FrameIndex nFrms = std::min(m_pSeq->GetFramesNumber(), m_pSeq->GetCamerasNumber());
	if(nFrms == 0)
		return;
	...
	if(m_drawCamType == DRAW_PATH_AND_ACTIVE_FRAME && IsFrameSolved(m_iFrmActive))
	{
		...
		const Sequence::IntrinsicType intrinsicType = m_pSeq->GetIntrinsicType();
		if(intrinsicType == Sequence::INTRINSIC_CONSTANT)
			m_camVolume.SetScale(m_camVolumeScale * m_pSeq->GetIntrinsicRectification().f());
		else if(intrinsicType == Sequence::INTRINSIC_VARIABLE)
			m_camVolume.SetScale(m_camVolumeScale * m_pSeq->GetIntrinsicRectification(m_iFrmActive).f());
	...
	if(m_drawCamType == DRAW_PATH_AND_ALL_KEY_FRAMES)
	{
		...
		{
			if(!IsFrameSolved(iFrm) || !(m_pSeq->GetFrameState(iFrm) & FLAG_FRAME_STATE_KEY_FRAME))
				continue;
			if(intrinsicType == Sequence::INTRINSIC_VARIABLE)
				m_camVolume.SetScale(m_camVolumeScale * m_pSeq->GetIntrinsicRectification(iFrm).f());
...
void ViewerSequence::DrawSelectedScenePoint()
{
	...
	Point3D center1, center2;
	m_pSeq->ComputePointMinimalRayAngleDot(m_iTrkSelected, m_rayDirs, iFrm1, iFrm2, false);
...
void ViewerSequence::DrawActiveScenePoints()
{
	...
	for(MeasurementIndex iMea = iMea1; iMea < iMea2; ++iMea)
	{
		if((iTrk = m_pSeq->GetMeasurementTrackIndex(iMea)) == INVALID_TRACK_INDEX || !(m_pSeq->GetTrackState(iTrk) & FLAG_TRACK_STATE_SOLVED))
			...
			glColor3ub(COLOR_ACTIVE_POINT_OUTLIER_R, COLOR_ACTIVE_POINT_OUTLIER_G, COLOR_ACTIVE_POINT_OUTLIER_B);
		else
			glColor3ub(COLOR_ACTIVE_POINT_INLIER_R, COLOR_ACTIVE_POINT_INLIER_G, COLOR_ACTIVE_POINT_INLIER_B);
		glVertex3fv(m_pSeq->GetPoint(iTrk));
	}
	glEnd();
}
...
void ViewerSequence::DrawFrameBar()
{
	...
	const float ratio = float(m_pWnd->size().x - FRAME_BAR_POINTER_WIDTH) / (m_pSeq->GetFramesNumberTotal() - 1);
	const Point2D trkBarPointerSize(ratio * 0.5f, TRACK_BAR_POINTER_HEIGHT);
	float y = m_pWnd->size().y - 1 - (STRING_BORDER_Y + STRING_SIZE_LARGE) - STRING_BORDER_Y;
	float x;
	...
	if(m_iTrkSelected != INVALID_TRACK_INDEX)
	{
		...
		glBegin(GL_QUADS);
		const MeasurementIndexList &iMeas = m_pSeq->GetTrackMeasurementIndexList(m_iTrkSelected);
	...
	else
	{
		...
		case DRAW_TRACK_FROM_LAST_FRAME:
			...
			break;
		case DRAW_TRACK_FROM_LAST_KEY_FRAME:
			for(iFrm1 = m_iFrmActive - 1; iFrm1 < nFrms && !(m_pSeq->GetFrameState(iFrm1) & FLAG_FRAME_STATE_KEY_FRAME); --iFrm1);
			iFrm2 = m_iFrmActive;
			break;
		case DRAW_TRACK_TO_NEXT_KEY_FRAME:
			iFrm1 = m_iFrmActive;
			for(iFrm2 = m_iFrmActive + 1; iFrm2 < nFrms && !(m_pSeq->GetFrameState(iFrm2) & FLAG_FRAME_STATE_KEY_FRAME); ++iFrm2);
			break;
		}
...
void ViewerSequence::DrawFrameState()
{
	...
	if(frmState & FLAG_FRAME_STATE_SOLVED)
		DrawStringCurrentLarge(", inliers = %d, inlier ratio = %.2f, inlier area ratio = %.2f, MSE = %.2f", m_nInliers, m_inlierRatio, m_inlierRatioArea, m_MSE);
}
...
bool ViewerSequence::DragFrameBar(const CVD::ImageRef &from, const CVD::ImageRef &to)
{
	const GLint y2 = m_pWnd->size().y - 1 - (STRING_BORDER_Y + STRING_SIZE_LARGE) - STRING_BORDER_Y + FRAME_BAR_POINTER_HEIGHT;
	...
	const FrameIndex iFrmActive = FrameIndex(GLfloat(x - pointerSizeHalf) * m_pSeq->GetFramesNumberTotal() / (m_pWnd->size().x - FRAME_BAR_POINTER_WIDTH));
	PrepareActiveFrame(iFrmActive);
	return true;
}
...
bool ViewerSequence::ShouldTrackBeHidden(const TrackIndex &iTrk)
{
	...
	return m_drawPtTypes[m_imgView] == DRAW_INLIER_POINTS && outlier || m_drawPtTypes[m_imgView] == DRAW_OUTLIER_POINTS && !outlier 
...
bool ViewerSequence::ShouldMeasurementBeHidden(const MeasurementIndex &iMea)
{
	...
	return iTrk != INVALID_TRACK_INDEX && ShouldTrackBeHidden(iTrk) || m_drawPtTypes[m_imgView] == DRAW_INLIER_POINTS && outlier 
		|| m_drawPtTypes[m_imgView] == DRAW_OUTLIER_POINTS && !outlier;
}

bool ViewerSequence::IsTrackOutlier(const TrackIndex &iTrk)
{
	return !(m_pSeq->GetTrackState(iTrk) & FLAG_TRACK_STATE_INLIER) || (m_pSeq->GetTrackState(iTrk) & FLAG_TRACK_STATE_COMMON_OUTLIER);
}

bool ViewerSequence::IsMeasurementOutlier(const MeasurementIndex &iMea)
{
	const TrackIndex iTrk = m_pSeq->GetMeasurementTrackIndex(iMea);
	return iTrk != INVALID_TRACK_INDEX && IsTrackOutlier(iTrk) || (m_pSeq->GetMeasurementState(iMea) & FLAG_MEASUREMENT_STATE_OUTLIER);
}

void ViewerSequence::ConvertWindowCoordinate2DTo3D(const CVD::ImageRef &where2D, Point3D &where3D)
{
	double X, Y, Z;
	//gluUnProject(where2D.x, m_pWnd->size().y - where2D.y, 0, m_modelMatrix, m_I, m_viewport, &X, &Y, &Z);
	gluUnProject(where2D.x, m_pWnd->size().y - where2D.y, 0, m_modelMatrix, m_projMatrix, m_viewport, &X, &Y, &Z);
...
void ViewerSequence::ConvertWindowCoordinate2DTo3D(const CVD::ImageRef &where2D, Point3D &where3D, const float &zPlane)
{
	double X, Y, Z;
	//gluUnProject(where2D.x, m_pWnd->size().y - where2D.y, 0, m_modelMatrix, m_I, m_viewport, &X, &Y, &Z);
	gluUnProject(where2D.x, m_pWnd->size().y - where2D.y, 0, m_modelMatrix, m_projMatrix, m_viewport, &X, &Y, &Z);
...
void ViewerSequence::ResetSceneViewProjectionMatrix()
{
	...
	glPushMatrix();
	glLoadIdentity();
	glTranslatef((K.cx() + K.cx()) / m_pSeq->GetImageWidth() - 1, 1 - (K.cy() + K.cy()) / m_pSeq->GetImageHeight(), 0.0f);
...
{
	m_factorImgToWin.Set(float(m_pWnd->size().x) / m_pSeq->GetImageWidth(), float(m_pWnd->size().y) / m_pSeq->GetImageHeight());
...
void ViewerSequence::ComputeDepthMeanAndRange(float &depthMean, float &depthRange)
{
...
{
	const float xWinSize = m_pSeq->GetImageWidth() * m_pSeq->GetIntrinsicMatrix().one_over_fx();
	const float yWinSize = m_pSeq->GetImageHeight() * m_pSeq->GetIntrinsicMatrix().one_over_fy();
...
bool ViewerSequence::OnKeyDown(const int key)
{
	switch(key)
	{
	case KEY_SWITCH_VIEW:					PrepareView(!m_imgView);				return true;
	case KEY_STEP_ACTIVE_FRAME_FORWARD:		PrepareActiveFrame(m_iFrmActive + 1);	return true;
	case KEY_STEP_ACTIVE_FRAME_BACKWARD:	PrepareActiveFrame(m_iFrmActive - 1);	return true;
	case KEY_STEP_DRAW_POINT_TYPE: {
		switch(m_drawPtTypes[m_imgView])
		{
		case DRAW_NO_POINT:			m_drawPtTypes[m_imgView] = DRAW_ALL_POINTS;		printf("[DRAW_ALL_POINTS]\n");		break;
		case DRAW_ALL_POINTS:		m_drawPtTypes[m_imgView] = DRAW_INLIER_POINTS;	printf("[DRAW_INLIER_POINTS]\n");	break;
		case DRAW_INLIER_POINTS:	m_drawPtTypes[m_imgView] = DRAW_OUTLIER_POINTS;	printf("[DRAW_OUTLIER_POINTS]\n");	break;
		case DRAW_OUTLIER_POINTS:	m_drawPtTypes[m_imgView] = DRAW_NO_POINT;		printf("[DRAW_NO_POINT]\n");		break;
		}
		return true; }
	case KEY_STEP_SELECTED_TRACK_FRAME: {
		if(m_iTrkSelected != INVALID_TRACK_INDEX)
		{
			...
			const FrameIndex nCrsps = FrameIndex(iMeas.size());
			FrameIndex i;
			for(i = 0; i < nCrsps && m_pSeq->GetMeasurementFrameIndex(iMeas[i]) <= m_iFrmActive; ++i);
			if(i == nCrsps)
				i = 0;
			PrepareActiveFrame(m_pSeq->GetMeasurementFrameIndex(iMeas[i]));
		}
		return true; }
	case KEY_DRAW_ERRORS:			m_drawErrs = !m_drawErrs;					return true;
	case KEY_SAVE_VIEW: {
		char fileName[MAX_LINE_LENGTH];
		sprintf(fileName, "%s%s_view_frm_%04d.jpg", m_pSeq->GetDirectory().c_str(), m_imgView ? "img" : "scene", m_iFrmActive);
	...
	if(m_imgView)
	{
		...
		case KEY_STEP_PROJECTION_TYPE: {
			switch(m_drawProjType)
			{
			case DRAW_NO_PROJECTION:				m_drawProjType = DRAW_ALL_PROJECTIONS;			printf("[DRAW_ALL_PROJECTIONS]\n");				break;
			case DRAW_ALL_PROJECTIONS:				m_drawProjType = DRAW_ACTIVE_PROJECTIONS;		printf("[DRAW_ACTIVE_PROJECTIONS]\n");			break;
			case DRAW_ACTIVE_PROJECTIONS:			m_drawProjType = DRAW_NO_PROJECTION;			printf("[DRAW_NO_PROJECTION]\n");				break;
			}
			return true; }
		case KEY_STEP_DRAW_TRACK_TYPE: {
			...
			case DRAW_NO_TRACK:						m_drawTrkType = DRAW_TRACK_FROM_LAST_FRAME;		printf("[DRAW_TRACK_FROM_LAST_FRAME]\n");		break;
			case DRAW_TRACK_FROM_LAST_FRAME:		m_drawTrkType = DRAW_TRACK_FROM_LAST_KEY_FRAME;	printf("[DRAW_TRACK_FROM_LAST_KEY_FRAME]\n");	break;
			case DRAW_TRACK_FROM_LAST_KEY_FRAME:	m_drawTrkType = DRAW_TRACK_TO_NEXT_KEY_FRAME;	printf("[DRAW_TRACK_TO_NEXT_KEY_FRAME]\n");		break;
			case DRAW_TRACK_TO_NEXT_KEY_FRAME:		m_drawTrkType = DRAW_NO_TRACK;					printf("[DRAW_NO_TRACK]\n");					break;
			}
			return true; }
		case KEY_DRAW_DISTRIBUTION:		m_drawDistribution = !m_drawDistribution;	return true;
		}
	}
	else
	{
		switch(key)
		{
		case KEY_TRANSLATE_XY:	m_controlType = CONTROL_TRANSLATE_XY;	printf("[CONTROL_TRANSLATE_XY]\n");	return true;
		case KEY_TRANSLATE_Z:	m_controlType = CONTROL_TRANSLATE_Z;	printf("[CONTROL_TRANSLATE_Z]\n");	return true;
		case KEY_ROTATION:		m_controlType = CONTROL_ROTATION;		printf("[CONTROL_ROTATION]\n");		return true;
		case KEY_DRAW_ARCBALL:	m_drawArcball = !m_drawArcball;												return true;
		case KEY_STEP_DRAW_CAMERA_TYPE: {
			switch(m_drawCamType)
			{
			case DRAW_NO_CAMERA:				m_drawCamType = DRAW_PATH;						printf("[DRAW_PATH]\n");					break;
			case DRAW_PATH:						m_drawCamType = DRAW_PATH_AND_ACTIVE_FRAME;		printf("[DRAW_PATH_AND_ACTIVE_FRAME]\n");	break;
			case DRAW_PATH_AND_ACTIVE_FRAME:	m_drawCamType = DRAW_PATH_AND_ALL_KEY_FRAMES;	printf("[DRAW_PATH_AND_ALL_KEY_FRAMES]\n");	break;
			case DRAW_PATH_AND_ALL_KEY_FRAMES:	m_drawCamType = DRAW_NO_CAMERA;					printf("[DRAW_NO_CAMERA]\n");				break;
			}
			return true; }
		case KEY_DRAW_ACTIVE_POINTS:	m_drawActivePts = !m_drawActivePts;		return true;
		case KEY_RESET_TRANSFORMATION:	ResetSceneViewTransformation();			return true;
		...
		case KEY_CAMERA_VOLUME_SCALE_DECREASE:
			...
			m_camVolume.SetScale(m_camVolumeScale);
			return true;
		case KEY_CAMERA_PATH_WIDTH_INCREASE:	m_camPathWidth += 0.5f;		return true;
		case KEY_CAMERA_PATH_WIDTH_DECREASE:	m_camPathWidth -= 0.5f;		return true;
		}
	}
	return false;
}
...
void ViewerSequence::OnMouseDown(const CVD::ImageRef &where, const int button)
{
	...
	else
	{
		...
		case CONTROL_TRANSLATE_XY:
			...
			ConvertWindowCoordinate2DTo3D(where, m_whereMouseDown3DStart, m_arcball.GetCenterZ());
			break;
...
void ViewerSequence::OnMouseUp(const CVD::ImageRef &where, const int button)
{
	if(!m_imgView)
	{
		if(m_handler.IsMouseDraging() && (m_controlType == CONTROL_TRANSLATE_XY || m_controlType == CONTROL_TRANSLATE_Z))
		{
...
bool ViewerSequence::OnMouseDraging(const CVD::ImageRef &from, const CVD::ImageRef &to, const int button)
{
	...
	else
	{
		...
		case CONTROL_TRANSLATE_XY: {
			...
			m_translation.XYZx() = _mm_add_ps(_mm_add_ps(_mm_mul_ps(R.r_00_01_02_x(), _mm_set1_ps(whereMouseDown3D.X() - m_whereMouseDown3DStart.X())), 
														 _mm_mul_ps(R.r_10_11_12_x(), _mm_set1_ps(whereMouseDown3D.Y() - m_whereMouseDown3DStart.Y()))), 
														 m_translationStart.XYZx());
			break; }
		case CONTROL_TRANSLATE_Z: {
			//m_translation.Z() = m_translationStart.Z() + m_dZuint * (from.y - to.y);
			const RotationTransformation3D &R = m_arcball.GetRotation();
			m_translation.XYZx() = _mm_add_ps(_mm_mul_ps(R.r_20_21_22_x(), _mm_set1_ps(m_dZuint * (from.y - to.y))), m_translationStart.XYZx());
			break; }
		case CONTROL_ROTATION: {
			Point3D whereMouseDown3D;
...
bool ViewerSequence::OnMouseDoubleClicked(const CVD::ImageRef &where, const int button)
{
	const IntrinsicMatrix &K = m_pSeq->GetIntrinsicMatrix();
	const Point2D boxSize(FEATURE_SELECTION_BOX_SIZE * m_factorWinToImg.x(), FEATURE_SELECTION_BOX_SIZE * m_factorWinToImg.y());
	const Point2D click(where.x * m_factorWinToImg.x(), where.y * m_factorWinToImg.y());
	...
	if(m_imgView)
	{
		...
		for(MeasurementIndex iMea = iMea1; iMea < iMea2; ++iMea)
		{
			...
			if(xDist < boxSize.x() && yDist < boxSize.y() && (dist = std::max(xDist, yDist)) < minDist)
			{
				iMeaSelected = iMea;
				minDist = dist;
			}
