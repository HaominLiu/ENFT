...
void ViewerSequenceSet::Run(const SequenceSet &seqs, const SequenceIndex &iSeqActive)
{
...
void ViewerSequenceSet::Initialize(const SequenceSet &seqs, const SequenceIndex iSeqActive)
{
	...
	for(iSeg = 0; iSeg < nSegs; ++iSeg)
		m_pathClrsSeg[iSeg] = CVD::Rgb<ubyte>(Random::GenerateUbyte(), Random::GenerateUbyte(), Random::GenerateUbyte());
...
bool ViewerSequenceSet::PrepareActiveSequence(const SequenceIndex iSeqActive)
{
	...
	m_iTrkSelected = m_iTrkCmnSelected == INVALID_TRACK_INDEX ? INVALID_TRACK_INDEX : m_pSeqSet->SearchCommonTrackForIndividualTrack(m_iTrkCmnSelected, m_iSeqActive);
	m_iFrmActive = m_iFrmImg = m_iFrmScn = INVALID_FRAME_INDEX;
	PrepareActiveFrame((m_iTrkSelected == INVALID_TRACK_INDEX 
		|| m_pSeqSet->GetSequence(m_iSeqActive).SearchTrackForFrameMeasurementIndex(m_iTrkSelected, m_iFrmsActive[m_iSeqActive]) != INVALID_MEASUREMENT_INDEX) ? 
		m_iFrmsActive[m_iSeqActive] : m_pSeqSet->GetSequence(m_iSeqActive).GetTrackFirstFrameIndex(m_iTrkSelected));
...
bool ViewerSequenceSet::PrepareSelectedTrack(const TrackIndex iTrkSelected)
{
	...
	m_iTrkCmnSelected = m_iTrkSelected == INVALID_TRACK_INDEX ? INVALID_TRACK_INDEX : m_pSeqSet->GetIndividualTrackCommonTrack(m_iSeqActive, m_iTrkSelected);
...
void ViewerSequenceSet::DrawAllProjections()
{
	...
	const float f = intrinsicType == Sequence::INTRINSIC_VARIABLE ? m_pSeq->GetIntrinsicRectification(m_iFrmActive).f() : m_pSeq->GetIntrinsicRectification().f();
	...
	for(SequenceIndex iSeq = 0; iSeq < nSeqs; ++iSeq)
	{
		if(iSeq != m_iSeqActive && !(m_pSeqSet->GetSequenceState(iSeq) & FLAG_SEQUENCE_STATE_REGISTRED))
			continue;
		const Sequence &seq = m_pSeqSet->GetSequence(iSeq);
...
void ViewerSequenceSet::DrawSceneView()
{
	if(m_drawSeqType != DRAW_ACTIVE_SEQUENCE)
	{
		...
		for(SequenceIndex iSeq = 0; iSeq < nSeqs; ++iSeq)
		{
			if(iSeq == m_iSeqActive || m_drawSeqType == DRAW_REGISTERED_SEQUENCES_INACTIVE_GRAY && !(m_pSeqSet->GetSequenceState(iSeq) & FLAG_SEQUENCE_STATE_REGISTRED))
				continue;
			m_iSeqActive = iSeq;
...
void ViewerSequenceSet::DrawSceneCameras()
{
	...
	Point3D center;
	const SegmentIndex iSegStart = m_mapSeqFrmToSeg[m_iSeqActive][0], iSegEnd = m_mapSeqFrmToSeg[m_iSeqActive][nFrms - 1];
	for(SegmentIndex iSeg = iSegStart; iSeg <= iSegEnd; ++iSeg)
	{
		const CVD::Rgb<ubyte> &clr = m_graySeqPath ? clrGray : (m_clrSegs ? m_pathClrsSeg[iSeg] : m_pathClrsSeq[m_iSeqActive]);
...
void ViewerSequenceSet::DrawSelectedScenePoint()
{
	...
	else
	{
		...
		const SequenceTrackIndexList &iSeqTrksIdv = m_pSeqSet->GetCommonTrackIndividualTrackIndexList(m_iTrkCmnSelected);
...
void ViewerSequenceSet::DrawActiveScenePoints()
{
	if(m_drawErrs)
	{
		...
		{
			if((iTrkIdv = m_pSeq->GetMeasurementTrackIndex(iMea)) == INVALID_TRACK_INDEX || !(m_pSeq->GetTrackState(iTrkIdv) & FLAG_TRACK_STATE_SOLVED)
			|| (iTrkCmn = m_pSeqSet->GetIndividualTrackCommonTrack(m_iSeqActive, iTrkIdv)) == INVALID_TRACK_INDEX)
				continue;
			const Point3D &Xcmn = m_pSeqSet->GetCommonPoint(iTrkCmn);
			const SequenceTrackIndexList &iSeqTrksIdv = m_pSeqSet->GetCommonTrackIndividualTrackIndexList(iTrkCmn);
			const SequenceIndex nCrsps = SequenceIndex(iSeqTrksIdv.size());
			for(SequenceIndex i = 0; i < nCrsps; ++i)
			{
				iSeqTrksIdv[i].Get(iSeq, iTrkIdv);
...
void ViewerSequenceSet::DrawFrameBar()
{
	...
	const float ratio = float(m_pWnd->size().x - FRAME_BAR_POINTER_WIDTH) / (m_mapCatFrmToSeqFrm.size() - 1);
	...
	for(SequenceIndex i = 0; i < nSeqsCC; ++i)
	{
		...
		const Point2D x1(pointerSizeHalf + (iFrmsCatCC.front() - 0.5f) * ratio, y - TRACK_BAR_POINTER_HEIGHT);
		const Point2D x2(pointerSizeHalf + (iFrmsCatCC.back() + 0.5f) * ratio, y + TRACK_BAR_POINTER_HEIGHT);
		DrawQuad(x1, x2);
	}
	glEnd();
	...
	if(m_iTrkSelected != INVALID_TRACK_INDEX)
	{
		...
		{
			x = pointerSizeHalf + iFrmsCat[m_pSeq->GetMeasurementFrameIndex(iMeas[i])] * ratio;
	...
	DrawStringBottomLeftLarge("Sequence %d, Segment %d, Frame %d", m_iSeqActive, m_mapSeqFrmToSeg[m_iSeqActive][m_iFrmActive], m_iFrmActive);
...
bool ViewerSequenceSet::DragFrameBar(const CVD::ImageRef &from, const CVD::ImageRef &to)
{
	...
	const uint iFrmCat = uint(GLfloat(x - pointerSizeHalf) * m_mapCatFrmToSeqFrm.size() / (m_pWnd->size().x - FRAME_BAR_POINTER_WIDTH));
...
bool ViewerSequenceSet::OnKeyDown(const int key)
{
	...
	case KEY_STEP_SELECTED_COMMON_TRACK_SEQUENCE: {
		if(m_iTrkCmnSelected == INVALID_TRACK_INDEX)
		{
			for(SequenceIndex iSeqActive = m_iSeqActive + 1; iSeqActive != m_iSeqActive; ++iSeqActive)
			{
				if(iSeqActive == m_pSeqSet->GetSequencesNumber())
					iSeqActive = 0;
				if(m_drawSeqType != DRAW_REGISTERED_SEQUENCES_INACTIVE_GRAY || (m_pSeqSet->GetSequenceState(iSeqActive) & FLAG_SEQUENCE_STATE_REGISTRED))
				{
					PrepareActiveSequence(iSeqActive);
					break;
				}
		...
		else
		{
			...
			for(j = i + 1; j != i; ++j)
			{
				if(j == nCrsps)
					j = 0;
				if(m_drawSeqType != DRAW_REGISTERED_SEQUENCES_INACTIVE_GRAY || (m_pSeqSet->GetSequenceState(iSeqTrksIdv[j].GetSequenceIndex()) & FLAG_SEQUENCE_STATE_REGISTRED))
				{
	...
	case KEY_STEP_DRAW_SEQUENCE_TYPE: {
		...
		{
		case DRAW_ACTIVE_SEQUENCE:						m_drawSeqType = DRAW_REGISTERED_SEQUENCES_INACTIVE_GRAY;	printf("[DRAW_REGISTERED_SEQUENCES_INACTIVE_GRAY]\n");	break;
		case DRAW_REGISTERED_SEQUENCES_INACTIVE_GRAY:	m_drawSeqType = DRAW_ALL_SEQUENCES_INACTIVE_GRAY;			printf("[DRAW_ALL_SEQUENCES_INACTIVE_GRAY]\n");			break;
		case DRAW_ALL_SEQUENCES_INACTIVE_GRAY:			m_drawSeqType = DRAW_ALL_SEQUENCES;							printf("[DRAW_ALL_SEQUENCES]\n");						break;
		case DRAW_ALL_SEQUENCES:						m_drawSeqType = DRAW_ACTIVE_SEQUENCE;						printf("[DRAW_ACTIVE_SEQUENCE]\n");						break;
		}
		return true; }
	case KEY_COLOR_SEGMENTS: m_clrSegs = !m_clrSegs; return true;
	}
	return ViewerSequence::OnKeyDown(key);
}

bool ViewerSequenceSet::OnMouseDoubleClicked(const CVD::ImageRef &where, const int button)
{
