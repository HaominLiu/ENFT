...
protected:
	...
	{
		glVertex2f(x - boxSize, y - boxSize);		glVertex2f(x - boxSize, y + boxSize);
		glVertex2f(x - boxSize, y + boxSize);		glVertex2f(x + boxSize, y + boxSize);
		glVertex2f(x + boxSize, y + boxSize);		glVertex2f(x + boxSize, y - boxSize);
		glVertex2f(x + boxSize, y - boxSize);		glVertex2f(x - boxSize, y - boxSize);
	}
	...
	{
		glVertex2f(x - boxSize.x(), y - boxSize.y());		glVertex2f(x - boxSize.x(), y + boxSize.y());
		glVertex2f(x - boxSize.x(), y + boxSize.y());		glVertex2f(x + boxSize.x(), y + boxSize.y());
		glVertex2f(x + boxSize.x(), y + boxSize.y());		glVertex2f(x + boxSize.x(), y - boxSize.y());
		glVertex2f(x + boxSize.x(), y - boxSize.y());		glVertex2f(x - boxSize.x(), y - boxSize.y());
	}
	...
	{
		glVertex2f(start.x(), start.y());	glVertex2f(start.x(), end.y());
		glVertex2f(start.x(), end.y());		glVertex2f(end.x(), end.y());
		glVertex2f(end.x(), end.y());		glVertex2f(end.x(), start.y());
		glVertex2f(end.x(), start.y());		glVertex2f(start.x(), start.y());
	}
	...
	inline void DrawTexture(const ushort &width, const ushort &height) const
	{
		...
		glTexCoord2s(0, 0);				glVertex2i(0, 0);
		glTexCoord2s(0, height);		glVertex2i(0, m_pWnd->size().y);
		glTexCoord2s(width, height);	glVertex2i(m_pWnd->size().x, m_pWnd->size().y);
		glTexCoord2s(width, 0);			glVertex2i(m_pWnd->size().x, 0);
		glEnd();
	...
	inline void DrawString()																				{ OnDrawString(); }
	inline bool KeyDown				(const int key)															{ return OnKeyDown(key); }
	inline bool KeyUp				(const int key)															{ return OnKeyUp(key); }
	inline void MouseMove			(const CVD::ImageRef &where)											{ OnMouseMove(where); }
	inline void MouseDown			(const CVD::ImageRef &where, const int button)							{ OnMouseDown(where, button); }
	inline void MouseUp				(const CVD::ImageRef &where, const int button)							{ OnMouseUp(where, button); }
	inline bool MouseDraging		(const CVD::ImageRef &from, const CVD::ImageRef &to, const int button)	{ return OnMouseDraging(from, to, button); }
	inline bool MouseDoubleClicked	(const CVD::ImageRef &where, const int button)							{ return OnMouseDoubleClicked(where, button); }
	inline void Resize				(const CVD::ImageRef &size)												{ ProgramGL::FitViewportWindows(size.x, size.y); OnResize(size); }
...
protected:
	...
	virtual bool OnKeyDown				(const int key)															{ return false; }
	virtual bool OnKeyUp				(const int key)															{ return false; }
	...
	virtual bool OnMouseDraging			(const CVD::ImageRef &from, const CVD::ImageRef &to, const int button)	{ return false; }
	virtual bool OnMouseDoubleClicked	(const CVD::ImageRef &where, const int button)							{ return false; }
