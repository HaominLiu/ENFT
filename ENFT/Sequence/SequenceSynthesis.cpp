...
//#endif
	...
	const __m128 loopSize4 = _mm_set1_ps(loopSize);
	...
	for(FrameIndex iFrm = 0; iFrm < nFrms; ++iFrm, angle += angleIncr)
	{
		...
		RTt.XYZx() = _mm_add_ps(_mm_mul_ps(C.r_20_21_22_x(), loopSize4), dC.XYZx());
...
void Sequence::SynthesizeTransformation(const float percentRigid, const float maxScale, SimilarityTransformation3D &S) const
{
	...
	Point3D mean;
	mean.XYZx() = _mm_mul_ps(_mm_set1_ps(1.0f / nTrks), sum.XYZx());
	...
	for(TrackIndex iTrk = 0; iTrk < nTrks; ++iTrk)
	{
		...
		distSqs[iTrk] = SSE::Sum012(_mm_mul_ps(dX.XYZx(), dX.XYZx()));
	}
...
//#endif
	...
	const __m128 tI3 = _mm_setr_ps(-f * p0, -f * p1, -p2, 1.0f);
	...
	for(TrackIndex iTrk = 0; iTrk < nTrks; ++iTrk)
	{
		...
		Xp.Y() = f * Xm.Y();
		Xp.Z() = Xm.Z();
		Xp *= 1 / SSE::Sum0123(_mm_mul_ps(tI3, Xm.XYZx()));
		Xp.reserve() = 1.0f;
#ifdef _DEBUG_WITH_EIGEN
		EigenVector X1, X2;
		SetV(Xm, X1);
...
void Sequence::PushBackProjectiveMatrixError(const ProjectiveMatrix &P1, const ProjectiveMatrix &P2, std::vector<float> &absErrs, std::vector<float> &relErrs)
{
	...
	const __m128 s1 = _mm_set1_ps(1 / P1.FrobeniusNorm()), s2 = _mm_set1_ps(1 / P2.FrobeniusNorm());
	p1.v0123() = _mm_mul_ps(s1, P1.M_00_01_02_03());		p2.v0123() = _mm_mul_ps(s2, P2.M_00_01_02_03());
	p1.v4567() = _mm_mul_ps(s1, P1.M_10_11_12_13());		p2.v4567() = _mm_mul_ps(s2, P2.M_10_11_12_13());
	p1.v8910x() = _mm_mul_ps(s1, P1.M_20_21_22_23());		p2.v8910x() = _mm_mul_ps(s2, P2.M_20_21_22_23());
