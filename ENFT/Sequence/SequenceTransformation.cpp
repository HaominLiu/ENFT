...
{
	Point3D center;
	center.XYZx() = size.XYZx() = _mm_setr_ps(0, 0, 0, 1);
...
void Sequence::TranslateScene(const Point3D &translation)
{
	if(translation.SquaredLength() == 0)
		return;
	const __m128 t = _mm_setr_ps(translation.X(), translation.Y(), translation.Z(), 1);
	Point3D dt;
	...
	const __m128 dX = _mm_setr_ps(translation.X(), translation.Y(), translation.Z(), 0);
	const TrackIndex nTrks = TrackIndex(m_Xs.Size());
	for(TrackIndex iTrk = 0; iTrk < nTrks; ++iTrk)
	{
		if(m_trkStates[iTrk] & FLAG_TRACK_STATE_SOLVED)
			m_Xs[iTrk].XYZx() = _mm_add_ps(m_Xs[iTrk].XYZx(), dX);
	}
}
...
void Sequence::ScaleScene(const float &scale)
{
	...
	const __m128 s = _mm_setr_ps(scale, scale, scale, 1);
	const TrackIndex nTrks = TrackIndex(m_Xs.Size());
	for(TrackIndex iTrk = 0; iTrk < nTrks; ++iTrk)
	{
		if(m_trkStates[iTrk] & FLAG_TRACK_STATE_SOLVED)
...
void Sequence::UpdateToMetric(const AbsoluteQuadric &Q)
{
	...
	const __m128 p = _mm_setr_ps(-Q.a0() * fI2, -Q.a1() * fI2, -Q.a2(), 1.0f);
...
//#endif
	...
	for(iTrk = 0; iTrk < nTrks; ++iTrk)
	{
		...
		Xm.Y() = fI * Xp.Y();
		Xm.Z() = Xp.Z();
		Xm *= 1 / SSE::Sum0123(_mm_mul_ps(p, Xp.XYZx()));
//#if _DEBUG
//		Xm.XYZx() = _mm_sub_ps(_mm_setzero_ps(), Xm.XYZx());
//		Xm.reserve() = 1.0f;
//#endif
		Xm.reserve() = 1.0f;
	}
