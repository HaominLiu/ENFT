...
void FeatureTracker::Initialize(const Sequence &seq, const std::string paramFileName)
{
	Configurator param;
...
{
	const ushort width = seq.GetImageWidth(), height = seq.GetImageHeight();
	...
	const float errThHomo = param.GetArgument("error_threshold_homography", 2.0f);
	m_Festor.m_ransacErrorThreshold = errThEp * errThEp;
	m_Hestor.m_ransacErrorThreshold = errThHomo * errThHomo;

	m_epInliersMinNum = ushort(param.GetArgument("epipolar_min_inliers_number", 20));
	...
	m_Festor.SetImageSize(width, height);
	m_Fdata.SetImageSize(width, height);
	...
	m_Hestor.m_ransacMinNumIters = uint(param.GetArgument("ransac_min_iterations_number_homography", 100));
	m_Hestor.m_ransacMaxNumIters = uint(param.GetArgument("ransac_max_iterations_number_homography", 300));

	m_texRGB.Generate(width, height);
	m_texGray.Generate(width, height);
	...
	const ushort nLevelsDoG = ushort(param.GetArgument("sift_dog_levels_number", 3));
	...
	const ushort searchRangeKeyToCurrent = m_bruteForceMatching ? USHRT_MAX : 
		ushort(param.GetArgument("sift_matching_search_range_key_to_current_frame", USHRT_MAX));
	const ushort searchRangeLastToCurrent = m_bruteForceMatching ? USHRT_MAX : 
		ushort(param.GetArgument("sift_matching_search_range_last_to_current_frame", USHRT_MAX));
	...
	m_enftMaxNumFtrs = m_siftMaxNumFtrs * 2;	// (key frame --> current frame) + (last frame --> current frame)
	m_ftrExtractorSift.Initialize(width, height, bufferSizeExtended, m_siftMaxNumFtrs, m_enftMaxNumFtrs, ftrTexWidth, nOctaves, nLevelsDoG, upSample, 
	...
	m_enftMinNumPtsPerPlane = ushort(param.GetArgument("enft_min_points_number_per_plane", 20));
	m_ftrMatcherSift.Initialize(m_maxNumFtrs, m_maxNumFtrs, ftrTexWidth, errThEp, nearest1To2RatioTh, descDotTh, 
		searchRangeKeyToCurrent, searchRangeLastToCurrent);
	m_ftrTrackerEnft.Initialize(width, height, m_bufferSize, m_maxNumFtrs, ftrTexWidth, winSz, nIters, lambdaEp, lambdaHomo, 
	...
	m_bucket.Initialize(width, height, ushort(param.GetArgument("flow_voting_bin_size_x", 10)), ushort(param.GetArgument("flow_voting_bin_size_y", 10)));
...
{
	if(outputFileName != "" && seq.LoadB(outputFileName.c_str()))
		return;

	const MeasurementIndex nMeasMax = MeasurementIndex(m_siftMaxNumFtrs) * MeasurementIndex(seq.GetFramesNumberTotal());
	seq.ReserveMeasurements(nMeasMax);
#if DESCRIPTOR_TRACK == 0
	seq.ReserveDescriptors(nMeasMax);
#endif
...
void FeatureTracker::RunSequentialTracking(Sequence &seq, FILE *fpTiming)
{
	...
	//FundamentalMatrix FKeyToLast;

	//LoadB("E:/tmp/test_163.txt", seq, iFrmKey, iFrmLast, iFrmCurrent);
	while(iFrmCurrent < nFrms)
	{
		//if(iFrmCurrent == 163)
		//{
		//	char fileName[MAX_LINE_LENGTH];
		//	sprintf(fileName, "E:/tmp/test_%d.txt", iFrmCurrent);
		//	SaveB(fileName, seq, iFrmKey, iFrmLast, iFrmCurrent);
		//	exit(0);
		//}
		//srand(5);
		//CVD::img_load(m_imgRGB, seq.GetImageFileName(iFrmCurrent));
		...
		else
		{
			...
			for(iFrmKey = iFrmLast - 1; iFrmKey < nFrms && CountFeatureMatchesKeyFrameToLastFrame(seq, iFrmKey, iFrmLast) > m_minNumMatchesKeyToLast; --iFrmKey);
			++iFrmKey;
			if(iFrmKey == iFrmLast)
				--iFrmKey;
			seq.MarkFrameKeyFrame(iFrmKey);
...
void FeatureTracker::RunBruteForceMatching(Sequence &seq, FILE *fpTiming)
{
	...
	//if(!seq.LoadB("ftrs.txt"))
	//{
	for(FrameIndex iFrm = 0; iFrm < nFrms; ++iFrm)
	{
		//CVD::img_load(m_imgRGB, seq.GetImageFileName(iFrm));
	...
	//exit(0);
	//}
	//printf("Feature tracking: ");	timer.PrintTotalAndStableMeanTiming(0);
	//exit(0);
	...
	for(iFrm2 = 0; iFrm2 < nFrms; ++iFrm2)
	{
		...
		//for(iFrm1 = iFrm2 - 1; iFrm1 < nFrms; --iFrm1)
		{
			...
			{
#if VERBOSE_FEATURE_TRACKING
				printf("\rFrame (%d, %d): NO matches!                              ", iFrm1, iFrm2);
#endif
				continue;
			}
			...
			pF = (FundamentalMatrix *) _aligned_malloc(sizeof(FundamentalMatrix), SSE_ALIGNMENT);
			*pF = F;
			FPool.push_back(pF);
			...
			for(ushort i = 0; i < nMatches; ++i)
			{
				...
				if(cntPositive < m_bfPositiveVotingTh && cntPositive < ushort((cntPositive + cntNegative) * m_bfPositiveVotingRatioTh + 0.5f))
					continue;
...
#if VERBOSE_FEATURE_TRACKING
			printf("\rFrame (%d, %d): %d - %d = %d matches      ", iFrm1, iFrm2, matches.size(), matches.size() - cntInliers, cntInliers);
#endif
	...
	if(fpTiming)
	{
		fprintf(fpTiming, "Feature extraction: ");	timer.PrintTotalAndStableMeanTiming(0, fpTiming);
		fprintf(fpTiming, "Feature matching:   ");	timer.PrintTotalAndStableMeanTiming(1, fpTiming);
	}
	else
	{
		printf("Feature extraction: ");				timer.PrintTotalAndStableMeanTiming(0);
		printf("Feature matching:   ");				timer.PrintTotalAndStableMeanTiming(1);
	}
...
{
	const ushort iBuffer = m_bufferManager.GetDataBufferIndex(iFrm);
	const ushort nFtrsSift = m_ftrExtractorSift.GetFeaturesNumber(iBuffer), nFtrsTotal = ushort(m_ftrsBuffer[iBuffer].Size());
	seq.PushBackFrame(m_ftrsBuffer[iBuffer].Data(), m_descsBuffer[iBuffer].Data(), nFtrsTotal, nFtrsSift, false);
	PushBackTrackColors(iFrm, seq);
}

void FeatureTracker::PushBackFrameAndFeatureMatches2(const FrameIndex &iFrm1, const FrameIndex &iFrm2, Sequence &seq)
{
	const ushort iBuffer1 = m_bufferManager.GetDataBufferIndex(iFrm1), iBuffer2 = m_bufferManager.GetDataBufferIndex(iFrm2);
	const std::vector<FeatureEnftMatch> &matchesEnft = m_matchesBufferEnft[iBuffer1];
	...
	const ushort nFtrsSift = ushort(ftrs2.Size()), nFtrsEnft = ushort(matchesEnft.size()), nFtrsTotal = nFtrsSift + nFtrsEnft;
	...
	m_ftrExtractorSift.PushBackFeaturesAndDescriptorsFromCPU(iBuffer2, nFtrsEnft, ftrs2.Data() + nFtrsSift, descs2.Data() + nFtrsSift);
...
void FeatureTracker::PushBackFrameAndFeatureMatches3(const FrameIndex &iFrmKey, const FrameIndex &iFrmLast, const FrameIndex &iFrmCurrent, Sequence &seq)
{
	...
	const std::vector<FeatureEnftMatch> &matchesKeyToCurrentEnft = m_matchesBufferEnft[iBufferKey], &matchesLastToCurrentEnft = m_matchesBufferEnft[iBufferLast];
	const AlignedVector<Descriptor> &descsKey = m_descsBuffer[iBufferKey], &descsLast = m_descsBuffer[iBufferLast];

	FeatureMatchList &matchesKeyToCurrent = m_matchesBuffer[iBufferKey], &matchesLastToCurrent = m_matchesBuffer[iBufferLast];
	...
	const ushort nFtrsSift = ushort(ftrsCurrent.Size()), nFtrsEnft = ushort(matchesKeyToCurrentEnft.size() + matchesLastToCurrentEnft.size()), nFtrsTotal = nFtrsSift + nFtrsEnft;	
	...
	m_ftrExtractorSift.PushBackFeaturesAndDescriptorsFromCPU(iBufferCurrent, nFtrsEnft, ftrsCurrent.Data() + nFtrsSift, descsCurrent.Data() + nFtrsSift);
...
void FeatureTracker::SetFeatureBuffer(const Sequence &seq, const FrameIndex &iFrm)
{
	const ushort iBuffer = m_bufferManager.GetDataBufferIndex(iFrm);
...
ushort FeatureTracker::CountFeatureMatchesKeyFrameToLastFrame(const Sequence &seq, const FrameIndex &iFrmKey, const FrameIndex &iFrmLast)
{
#if _DEBUG
	...
	const FeatureIndex nFtrsKey = seq.GetFrameFirstMeasurementIndex(iFrmKey + 1) - iMeaStartKey;
	...
	{
		if((iTrk = iTrksKey[iFtrKey]) != INVALID_TRACK_INDEX && (iFrm = seq.GetMeasurementFrameIndex(seq.GetTrackMeasurementIndexList(iTrk).back())) == iFrmLast)
			++cnt;
	}
	return cnt;
}

void FeatureTracker::GetFeatureMatchesKeyFrameToLastFrame(const Sequence &seq, const FrameIndex &iFrmKey, const FrameIndex &iFrmLast, 
														  FeatureIndexList &iFtrsKeyToLast, FeatureIndexList &iFtrsLastToKey)
{
#if _DEBUG
	assert(iFrmLast == seq.GetFramesNumber() - 1);
#endif
	const MeasurementIndex iMeaStartKey = seq.GetFrameFirstMeasurementIndex(iFrmKey), iMeaStartLast = seq.GetFrameFirstMeasurementIndex(iFrmLast);
	const TrackIndex *iTrksKey = seq.GetMeasurementTrackMap().data() + iMeaStartKey;
	const FeatureIndex nFtrsKey = seq.GetFrameFirstMeasurementIndex(iFrmKey + 1) - iMeaStartKey;
	const FeatureIndex nFtrsLast = seq.GetFrameFirstMeasurementIndex(iFrmLast + 1) - iMeaStartLast;
...
void FeatureTracker::TrackFeatures2(const FrameIndex &iFrm1, const FrameIndex &iFrm2)
{
	...
	MatchFeaturesSift(iFrm1, iFrm2, F, matches, false);
	m_matchesEnft1.resize(0);
	TrackFeaturesEnft(iFrm1, iFrm2, F, matches, m_matchesEnft1, matchesEnft);
	RemoveOutlierMatchesFlowVoting(iFrm1, iFrm2, matches, matchesEnft);
...
static inline void PushBackMatches(const std::vector<Match> &matchesSrc, std::vector<Match> &matchesDst)
{
	matchesDst.insert(matchesDst.end(), matchesSrc.begin(), matchesSrc.end());
}

void FeatureTracker::TrackFeatures3(const FrameIndex &iFrmKey, const FrameIndex &iFrmLast, const FrameIndex &iFrmCurrent, const FeatureIndexList &iFtrsKeyToLast, 
									const FeatureIndexList &iFtrsLastToKey/*, const FundamentalMatrix &FKeyToLast*/)
{
	...
	printf("  Frame (%d, %d)\n", iFrmLast, iFrmCurrent);
#endif
	m_ftrMatcherSift.FromMatches13To23(iFtrsKeyToLast, matchesKeyToCurrent, m_matches2);
	...
	//RemoveConflictedMatchesSift(iFrmKey, iFrmLast, iFrmCurrent, FKeyToLast, matchesKeyToCurrent, matchesLastToCurrent);
	const ushort nMatchesSiftExclusiveKeyToCurrent = ushort(matchesKeyToCurrent.size()), nMatchesSiftExclusiveLastToCurrent = ushort(matchesLastToCurrent.size());
	...
	std::vector<FeatureEnftMatch> &matchesLastToCurrentEnft = m_matchesBufferEnft[iBufferLast];
	m_matchesEnft1.resize(0);
	TrackFeaturesEnft(iFrmLast, iFrmCurrent, FLastToCurrent, matchesLastToCurrent, m_matchesEnft1, matchesLastToCurrentEnft);
	...
	std::vector<FeatureEnftMatch> &matchesKeyToCurrentEnft = m_matchesBufferEnft[iBufferKey];
	...
	TrackFeaturesEnft(iFrmKey, iFrmCurrent, FKeyToCurrent, matchesKeyToCurrent, m_matchesEnft1, matchesKeyToCurrentEnft);
	matchesKeyToCurrent.resize(nMatchesSiftExclusiveKeyToCurrent);

#if VERBOSE_FEATURE_TRACKING >= 2
	printf("  Frame (%d, %d)\n", iFrmLast, iFrmCurrent);
#endif
	m_ftrTrackerEnft.FromMatches13To23(iFtrsKeyToLast, matchesKeyToCurrentEnft, m_matchesEnft2);
	const ushort nMatchesEnftExclusiveKeyToCurrent = ushort(matchesKeyToCurrentEnft.size());
	RemoveOutlierMatchesEnft(iFrmLast, iFrmCurrent, FLastToCurrent, m_matchesEnft2);
	const ushort nMatchesEnftExclusiveLastToCurrent = ushort(matchesLastToCurrentEnft.size());
...
#if VERBOSE_FEATURE_TRACKING >= 2
	printf("----------------------------------------------------------------\n");
	printf("  SIFT matches: (%d + (%d) + %d) = %d\n", nMatchesSiftExclusiveKeyToCurrent, nMatchesSiftCommon, nMatchesSiftExclusiveLastToCurrent, 
		nMatchesSiftExclusiveKeyToCurrent + nMatchesSiftCommon + nMatchesSiftExclusiveLastToCurrent);
	printf("  ENFT features: (%d + (%d) + %d) = %d\n", nMatchesEnftExclusiveKeyToCurrent, nMatchesEnftCommon, nMatchesEnftExclusiveLastToCurrent, 
		nMatchesEnftExclusiveKeyToCurrent + nMatchesEnftCommon + nMatchesEnftExclusiveLastToCurrent);
#endif

	RemoveOutlierMatchesFlowVoting(iFrmLast, iFrmCurrent, matchesLastToCurrent, matchesLastToCurrentEnft);
...
#if VERBOSE_FEATURE_TRACKING/* == 1*/
	printf("----------------------------------------------------------------\n");
	printf("  SIFT matches: %d\n", matchesKeyToCurrent.size() + matchesLastToCurrent.size());
	printf("  ENFT matches: %d\n", matchesKeyToCurrentEnft.size() + matchesLastToCurrentEnft.size());
	printf("  Frame (%d, %d) matches: %d\n", iFrmKey, iFrmCurrent, matchesKeyToCurrent.size() + matchesKeyToCurrentEnft.size());
	printf("  Frame (%d, %d) matches: %d\n", iFrmLast, iFrmCurrent, matchesLastToCurrent.size() + matchesLastToCurrentEnft.size() + m_matches1.size() + m_matchesEnft1.size());
#endif
}