...
void FeatureTracker::SaveB(const char *fileName, const Sequence &seq, const FrameIndex &iFrmKey, const FrameIndex &iFrmLast, const FrameIndex &iFrmCurrent) const
{
	FILE *fp = fopen(fileName, "wb");
	seq.SaveB(fp);
	fwrite(&iFrmKey, sizeof(FrameIndex), 1, fp);		fwrite(&iFrmLast, sizeof(FrameIndex), 1, fp);		fwrite(&iFrmCurrent, sizeof(FrameIndex), 1, fp);
...
void FeatureTracker::LoadB(const char *fileName, Sequence &seq, FrameIndex &iFrmKey, FrameIndex &iFrmLast, FrameIndex &iFrmCurrent)
{
	...
	seq.LoadB(fp);
	fread(&iFrmKey, sizeof(FrameIndex), 1, fp);		fread(&iFrmLast, sizeof(FrameIndex), 1, fp);		fread(&iFrmCurrent, sizeof(FrameIndex), 1, fp);
