...
void TrackMatcher::MatchTracksEnft(/*const */Sequence &seq, const std::vector<TrackIndexList> &iTrkClusters, TrackMatchList &trkMatches)
{
	...
	for(iterInit = iRegion = 0; iterInit < m_enftMaxNumItersInitMatchingMatrix; ++iterInit)
	{
		...
		if(iterInit == 0)
			...
			break;
		MatchFeaturesSift_VerifyMatches(seq, iFrm1Init, iFrm2Init, m_matchesExist, m_F, m_inliers, m_outliers, m_matchesNew);
		...
		for(iterUpd = 0; iterUpd < m_enftMaxNumItersUpdMatchingMatrix; ++iterUpd)
		{
			if(!m_matchingMatrix.ExtractUpdatedSeed(iFrm1Upd, iFrm2Upd, m_matchesExist, m_idxs))
				break;
			VerifyMatches_MatchNewFeaturesSift(seq, iFrm1Upd, iFrm2Upd, m_matchesExist, m_F, m_inliers, m_outliers, m_matchesNew);
...
void TrackMatcher::MatchTracksEnft(/*const */SequenceSet &seqs, const SequenceIndex &iSeq1, const SequenceIndex &iSeq2, const std::vector<TrackIndexListPair> &iTrkClusters, 
								   TrackMatchList &trkMatches)
{
	...
	for(iterInit = iRegion = 0; iterInit < m_enftMaxNumItersInitMatchingMatrix; ++iterInit)
	{
		...
		if(iterInit == 0)
			...
			break;
		MatchFeaturesSift_VerifyMatches(seqs, iSeq1, iSeq2, iFrm1Init, iFrm2Init, m_matchesExist, m_F, m_inliers, m_outliers, m_matchesNew);
		confidenceUpd = ushort(m_matchesNew.size());
#if VERBOSE_TRACK_MATCHING
		printf("\r  [%d] Frame (%d, %d): %d matches", iRegion, iFrm1Init, iFrm2Init, confidenceUpd);
#endif
		...
		for(iterUpd = 0; iterUpd < m_enftMaxNumItersUpdMatchingMatrix; ++iterUpd)
		{
			if(!m_matchingMatrix.ExtractUpdatedSeed(iFrm1Upd, iFrm2Upd, m_matchesExist, m_idxs))
				break;
			VerifyMatches_MatchNewFeaturesSift(seqs, iSeq1, iSeq2, iFrm1Upd, iFrm2Upd, m_matchesExist, m_F, m_inliers, m_outliers, m_matchesNew);
			confidenceUpd = ushort(m_inliers.size() + m_matchesNew.size());
#if VERBOSE_TRACK_MATCHING
			printf("\r      Frame (%d, %d): %d", iFrm1Upd, iFrm2Upd, confidenceUpd);
#endif
...
void TrackMatcher::MatchTracksEnft(/*const */SequenceSet &seqs, const SequenceIndex &iSeq1, const SequenceIndex &iSeq2, const SparseMatrix<float> &matchingMatrixInit, 
								   TrackMatchList &trkMatches)
{
#if VERBOSE_TRACK_MATCHING
	printf("Matching tracks\n");
