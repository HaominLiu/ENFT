...
void TrackMatcher::PrepareViewing(const SequenceSet &seqs, const SequenceIndex &iSeq1, const SequenceIndex &iSeq2, const std::vector<TrackIndexListPair> &iTrkClusters)
{
	if(!m_view)
		return;
...
void TrackMatcher::PrepareViewing(const SequenceSet &seqs, const SequenceIndex &iSeq1, const SequenceIndex &iSeq2, const SparseMatrix<float> &matchingMatrixInit)
{
	if(!m_view)
		return;
	const Sequence &seq1 = seqs[iSeq1], &seq2 = seqs[iSeq2];
...
void TrackMatcher::PrepareInitialMatchingMatrixImageAndTexture(const Sequence &seq1, const Sequence &seq2, const std::vector<TrackIndexListPair> &iTrkClusters)
{
	if(!m_view)
		return;
	memset(m_matchingMatrixInitImg.data(), 0, sizeof(float) * m_matchingMatrixInitImg.totalsize());
...
void TrackMatcher::PrepareUpdatingMatchingMatrixImageAndTexture(const Sequence &seq, const FrameIndex &iFrm1Upd, const FrameIndex &iFrm2Upd, const FeatureMatchList &matches)
{
	if(!m_view)
		return;
	//memset(m_matchingMatrixUpdImg.data(), 0, sizeof(ushort) * m_matchingMatrixUpdImg.totalsize());
