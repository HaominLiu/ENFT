...
void TrackMatcher::MatchFeaturesSift(/*const */Sequence &seq1, /*const */Sequence &seq2, const FrameIndex &iFrm1, const FrameIndex &iFrm2, 
									 std::vector<ScoredMatch> &matches)
{
	const FeatureIndex nFtrs1 = seq1.GetFrameFeaturesNumber(iFrm1), nFtrs2 = seq2.GetFrameFeaturesNumber(iFrm2);
...
void TrackMatcher::MatchFeaturesSift(/*const */Sequence &seq1, /*const */Sequence &seq2, const FrameIndex &iFrm1, const FrameIndex &iFrm2, 
									 const FeatureIndexList &iFtrs1, const FeatureIndexList &iFtrs2, std::vector<ScoredMatch> &matches)
{
	FeatureIndex iFtr1, iFtr2;
	const FeatureIndex nFtrs1 = FeatureIndex(iFtrs1.size()), nFtrs2 = FeatureIndex(iFtrs2.size());
...
void TrackMatcher::MatchFeaturesSift(/*const */Sequence &seq1, /*const */Sequence &seq2, const FrameIndex &iFrm1, const FrameIndex &iFrm2, 
									 const FeatureIndexList &iFtrs1, const FeatureIndexList &iFtrs2, const FundamentalMatrix &F, FeatureMatchList &matches)
{
	FeatureIndex iFtr1, iFtr2;
	const FeatureIndex nFtrs1 = FeatureIndex(iFtrs1.size()), nFtrs2 = FeatureIndex(iFtrs2.size());
	...
	m_ftrMatcherSift.MatchFeatures(m_ftrTex1, m_ftrTex2, m_descTex1, m_descTex2, nFtrs1, nFtrs2, F, true);
	m_ftrMatcherSift.DownloadMatches12ToCPU(nFtrs1, nFtrs2, matches);
	const ushort nMatches = ushort(matches.size());
	for(ushort i = 0; i < nMatches; ++i)
	{
...
void TrackMatcher::MatchFeaturesSift_VerifyMatches(/*const */Sequence &seq, const FrameIndex &iFrm1, const FrameIndex &iFrm2, /*const */FeatureMatchList &matchesExist, 
												   FundamentalMatrix &F, std::vector<ushort> &inliers, std::vector<ushort> &outliers, FeatureMatchList &matchesNew)
{
	FeatureIndex iFtr1, iFtr2;
	const FeatureIndex nFtrs1 = seq.GetFrameFeaturesNumber(iFrm1), nFtrs2 = seq.GetFrameFeaturesNumber(iFrm2);
	...
	{
		if((iFtr2 = seq.SearchTrackForFrameFeatureIndex(iTrks1[iFtr1], iFrm2)) != INVALID_FEATURE_INDEX)
	...
	{
		// Step1: SIFT matching
		MatchFeaturesSift(seq, seq, iFrm1, iFrm2, m_scoredMatches);

		// Step2: filter outliers by epipolar geometry
		m_Fdata.SetMatches(seq.GetFrameFeatures(iFrm1), seq.GetFrameFeatures(iFrm2), m_scoredMatches, m_orders);
	...
	{
		// Step1: SIFT matching for unmatched features
		...
		MatchFeaturesSift(seq, seq, iFrm1, iFrm2, m_iFtrs1Unmatched, m_iFtrs2Unmatched, m_scoredMatches);
		const ushort nMatchesNew = ushort(m_scoredMatches.size());
...
void TrackMatcher::MatchFeaturesSift_VerifyMatches(/*const */SequenceSet &seqs, const SequenceIndex &iSeq1, const SequenceIndex &iSeq2, const FrameIndex &iFrm1, 
												   const FrameIndex &iFrm2, const FeatureMatchList &matchesExist, FundamentalMatrix &F, 
												   std::vector<ushort> &inliers, std::vector<ushort> &outliers, FeatureMatchList &matchesNew)
{
	FeatureIndex iFtr1, iFtr2;
	...
	{
		// Step1: SIFT matching
		MatchFeaturesSift(seq1, seq2, iFrm1, iFrm2, m_scoredMatches);

		// Step2: filter outliers by epipolar geometry
		m_Fdata.SetMatches(seq1.GetFrameFeatures(iFrm1), seq2.GetFrameFeatures(iFrm2), m_scoredMatches, m_orders);
		m_Festor.RunLoProsac(m_Fdata, m_orders, F, inliers/*, 3*/);
		const ushort nInliers = ushort(inliers.size());
		matchesNew.resize(nInliers);
		for(ushort i = 0; i < nInliers; ++i)
...
//#endif
	...
	else
	{
		...
		MatchFeaturesSift(seq1, seq2, iFrm1, iFrm2, m_iFtrs1Unmatched, m_iFtrs2Unmatched, m_scoredMatches);
	...
	seqs.GetCameraPairEstimatorData(iSeq1, iSeq2, iFrm1, iFrm2, matches3D, m_CPdata);
	m_CPestor.m_ransacErrorThreshold = m_errSqThReprojCam * std::min(seq1.GetIntrinsicMatrix().one_over_fxy(), seq2.GetIntrinsicMatrix().one_over_fxy());
...
void TrackMatcher::VerifyMatches_MatchNewFeaturesSift(/*const */Sequence &seq, const FrameIndex &iFrm1, const FrameIndex &iFrm2, /*const */FeatureMatchList &matchesExist, 
													  FundamentalMatrix &F, std::vector<ushort> &inliers, std::vector<ushort> &outliers, 
													  FeatureMatchList &matchesNew)
{
	...
	m_Fdata.SetMatches(seq.GetFrameFeatures(iFrm1), seq.GetFrameFeatures(iFrm2), matchesExist);
...
//#endif
	...
	// Step2: SIFT matching for unmatched features & matched outlier features
	std::vector<bool> &ftrMarks1 = m_marks1, &ftrMarks2 = m_marks2;
	ftrMarks1.assign(nFtrs1, false);
	ftrMarks2.assign(nFtrs2, false);
	...
	MatchFeaturesSift(seq, seq, iFrm1, iFrm2, m_iFtrs1Unmatched, m_iFtrs2Unmatched, F, matchesNew);
...
void TrackMatcher::VerifyMatches_MatchNewFeaturesSift(/*const */SequenceSet &seqs, const SequenceIndex &iSeq1, const SequenceIndex &iSeq2, const FrameIndex &iFrm1, 
													  const FrameIndex &iFrm2, const FeatureMatchList &matchesExist, FundamentalMatrix &F, 
													  std::vector<ushort> &inliers, std::vector<ushort> &outliers, FeatureMatchList &matchesNew)
{
	// Step1: estimate fundamental matrix and detect outlier matches
	/*const */Sequence &seq1 = seqs[iSeq1], &seq2 = seqs[iSeq2];
	m_Fdata.SetMatches(seq1.GetFrameFeatures(iFrm1), seq2.GetFrameFeatures(iFrm2), matchesExist);
	m_Festor.RunLosac(m_Fdata, F, inliers);
	const ushort nMatchesExist = ushort(matchesExist.size());
	...
	MatchFeaturesSift(seq1, seq2, iFrm1, iFrm2, m_iFtrs1Unmatched, m_iFtrs2Unmatched, F, matchesNew);
